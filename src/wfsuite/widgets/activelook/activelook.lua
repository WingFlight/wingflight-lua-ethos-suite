...
local function getMode()
    ...
    local previewMode = wfsuite.session and wfsuite.session.activelookPreviewMode
    if appGuiRunning and (previewMode == "preflight" or previewMode == "inflight" or previewMode == "postflight") then
        return previewMode
    end
    local mode = wfsuite.flightmode and wfsuite.flightmode.current
...
getSensorValue = function(name)
    local telemetry = wfsuite.tasks and wfsuite.tasks.telemetry
...
local function readTimer(context, mode, now)
    local timer = wfsuite.session and wfsuite.session.timer
...
local function loadLayoutForState(stateKey)
    local prefs = wfsuite.preferences and wfsuite.preferences.activelook or {}
...
local function wakeup(context)
    if wfsuite.session and wfsuite.session.activelookReset then
        wfsuite.session.activelookReset = false
