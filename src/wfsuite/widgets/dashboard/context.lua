...
local cached = package.loaded["wfsuite.dashboard.context"]
...
local LIVE_SENSOR_CANDIDATES = {
  ...
  crsf = {
    ...
    -- detectSourceMode() in rotorflight-lua-ethos-suite's pre-rewrite tree).
    -- Older firmware without that marker never exposed subId=2 at all and
    -- instead used this subIdStart/subIdEnd range (see that same tree's
...
-- Restored from the pre-rewrite suite's widgets/dashboard/lib/utils.lua
-- (see `git show stab-curve:src/wfsuite/widgets/dashboard/lib/utils.lua`)
-- after the rfsuite-full-rewrite base collapsed this to a flat
-- color-fill-only stub, which silently broke every caller that passes a
-- style table (fillcolor/bordercolor/borderwidth/roundradius/inset*) as
-- `bgcolor` instead of a plain color -- a real, reproducible crash
-- (`lcd.color`: "number expected, got table"), not a hypothetical one:
-- the kevd theme's `rs_bgstyle`/`rs_govbgstyle`/`arcGroupTileBg` panels
-- all rely on it, and every gauge/dial object type
-- (objects/gauge/{arc,bar,ring,step}.lua, objects/dial/image.lua)
-- already reassigns `x, y, w, h = utils.drawBoxBackground(...)`
-- expecting the inset-adjusted content rect this returns.
...
-- it from wfsuite.config.maxModelImageBytes, which is never set), hence the
...
-- (src/main/fc/runtime_config.h in wingflight-firmware) -- that bit only ever
-- accompanies another, real reason (it's set when the arm switch is on while
...
package.loaded["wfsuite.dashboard.context"] = context
return context
