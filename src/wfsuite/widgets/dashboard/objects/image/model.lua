...
local loadImage = wfsuite.utils.loadImage
local isImageTooLarge = wfsuite.utils.isImageTooLarge
...
local function resolveModelImage(cfg)
    ...
    local craftName = wfsuite and wfsuite.session and wfsuite.session.craftName
    if craftName and craftName ~= "" then
        local cached = _imgCache[craftName]
        if cached == nil then
            local maxBytes = (wfsuite.config and wfsuite.config.maxModelImageBytes)
...
function render.paint(x, y, w, h, box)
    ...
    -- displayValue/unit are always nil here -- see objects/image/image.lua's
    -- paint() for why this call still pulls its weight (caches the title
    -- measurement, exposes the title-adjusted content rect for the image).
