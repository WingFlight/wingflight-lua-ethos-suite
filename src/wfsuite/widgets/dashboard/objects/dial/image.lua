...
wfsuite.session.dialImageCache = wfsuite.session.dialImageCache or {}
local function loadDialPanelCached(dialId)
    local key = tostring(dialId or "panel1")
    if not wfsuite.session.dialImageCache[key] then
        local panelPath = resolveDialAsset(dialId, "widgets/dashboard/gfx/dials") or "widgets/dashboard/gfx/dials/panel1.png"
        wfsuite.session.dialImageCache[key] = wfsuite.utils.loadImage(panelPath)
    end
    return wfsuite.session.dialImageCache[key]
end
