...
function render.dirty(box)
    if not wfsuite.session.telemetryState then return false end
...
function render.wakeup(box)
    ...
    local inPostflight = (wfsuite.flightmode and wfsuite.flightmode.current == "postflight")
    ...
    if telemetryActive and session and session.modelPreferences then
        value = wfsuite.ini.getvalue(session.modelPreferences, "general", "totalflighttime")
        lastValue = value
    else
    ...
    if haveNumber then
        ...
        local minutes = floor((value % 3600) / 60)
        local seconds = floor(value % 60)
        displayValue = format("%02d:%02d:%02d", hours, minutes, seconds)
    else
