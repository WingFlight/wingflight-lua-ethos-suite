...
local function getPref(key) return wfsuite.widgets.dashboard.getPreference(key) end

local function setPref(key, value) wfsuite.widgets.dashboard.savePreference(key, value) end

local formFields = {}
local prevConnectedState = nil

local function isTelemetryConnected() return wfsuite and wfsuite.session and wfsuite.session.isConnected and wfsuite.session.mcu_id and wfsuite.preferences end
...
local function configure()
    ...
    formFields[#formFields + 1] = form.addNumberField(voltage_min_line, nil, 50, 140, function()
    ...
    formFields[#formFields + 1] = form.addNumberField(voltage_max_line, nil, 50, 140, function()
