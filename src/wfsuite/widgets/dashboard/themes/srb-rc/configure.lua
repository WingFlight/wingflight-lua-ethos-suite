...
local function getPref(key)
    return wfsuite.widgets.dashboard.getPreference(key)
end
...
local function isTelemetryConnected()
    return wfsuite and wfsuite.session and wfsuite.session.isConnected and wfsuite.session.mcu_id and wfsuite.preferences
end
