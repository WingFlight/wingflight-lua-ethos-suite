...
local function readArmflags(telemetry, showReason)
    ...
    if showReason and disableflags ~= nil and wfsuite.utils.armingDisableFlagsToString then
        local reason = wfsuite.utils.armingDisableFlagsToString(floor(disableflags))
...
local function readTime(spec, session)
    ...
    elseif spec.timesource == "count" and session.modelPreferences then
        local value = wfsuite.ini.getvalue(session.modelPreferences, "general", "flightcount")
        return value ~= nil and tostring(value) or nil
    elseif spec.timesource == "total" and session.modelPreferences then
        local value = wfsuite.ini.getvalue(session.modelPreferences, "general", "totalflighttime")
...
local function readSpec(box, spec, telemetry, slotKey)
    ...
    local session = wfsuite.session or {}
