...
local DASHBOARD_GFX_DIR = "SCRIPTS:/" .. wfsuite.config.baseDir .. "/widgets/dashboard/gfx/"
...
local function fmtWfsuiteVersion()
    local v = wfsuite and wfsuite.config and wfsuite.config.version
    if type(v) ~= "table" then return "wfsuite ?" end
    local major = tonumber(v.major) or 0
    local minor = tonumber(v.minor) or 0
    local rev   = tonumber(v.revision) or 0
    local sfx   = v.suffix
    local base = format("WFSUITE %d.%d.%d", major, minor, rev)
    if sfx and sfx ~= "" then
...
local function loadLogoBitmap(themeState, isLegacyDark, panelBg)
    ...
    local bmp = wfsuite.utils.loadImage(variant == "light" and LOGO_LIGHT_PATH or LOGO_DARK_PATH)
