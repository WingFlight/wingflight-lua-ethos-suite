...
local function doErase()
    ...
    local function readDataflashSummary()
        if not (wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.api and wfsuite.tasks.msp.api.load) then return end
        local API = wfsuite.tasks.msp.api.load(DATAFLASH_SUMMARY_API)
        ...
        API.setCompleteHandler(function()
            ...
            if total ~= nil then wfsuite.session.bblSize = total end
            if used ~= nil then wfsuite.session.bblUsed = used end
            if flags ~= nil then wfsuite.session.bblFlags = flags end
