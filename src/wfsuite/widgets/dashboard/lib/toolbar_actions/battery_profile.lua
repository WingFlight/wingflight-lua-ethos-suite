...
function getDashboardUtils()
    local widgets = wfsuite and wfsuite.widgets
...
local function openProgressDialog(...)
    if wfsuite.utils.ethosVersionAtLeast({1, 7, 0}) and form.openWaitDialog then
        local arg1 = select(1, ...)
...
local function updateProgressMessage()
    ...
    local showMsp = wfsuite.preferences and wfsuite.preferences.general and wfsuite.preferences.general.mspstatusdialog
    local mspStatus = (showMsp and wfsuite.session and wfsuite.session.mspStatusMessage) or nil
    if showMsp then
...
local function buildBatteryProfileList()
    local profilesRaw = wfsuite.session.batteryConfig and wfsuite.session.batteryConfig.profiles
    local profileList = {}
    if profilesRaw then
...
local function setBatteryType(typeIndex, profileName)
    ...
    if typeIndex == wfsuite.session.activeBatteryType then
        if wfsuite.session.showConfirmationDialog then
            form.openDialog({
    ...
    local api = wfsuite.tasks.msp.api.load(BATTERY_PROFILE_API)

    api.setCompleteHandler(function()
        wfsuite.session.activeBatteryType = typeIndex
