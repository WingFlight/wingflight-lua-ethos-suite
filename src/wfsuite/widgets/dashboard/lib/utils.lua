...
local function getThemeLogoOverride()
    local widgetPath = wfsuite.widgets.dashboard.currentWidgetPath
    ...
    if src and folder then
        local themeBase
        if src == "user" then
            themeBase = "SCRIPTS:/" .. wfsuite.config.preferences .. "/dashboard/" .. folder .. "/"
        else
            themeBase = "SCRIPTS:/" .. wfsuite.config.baseDir .. "/widgets/dashboard/themes/" .. folder .. "/"
        end
...
local function supportsSystemThemeColors()
    if not _supportsThemeChecked and wfsuite and wfsuite.utils and wfsuite.utils.ethosVersionAtLeast then
        _supportsTheme = wfsuite.utils.ethosVersionAtLeast(ETHOS_THEME_MIN_VERSION) == true
...
function utils.isModelPrefsReady() return wfsuite and wfsuite.session and wfsuite.session.modelPreferences end
...
function utils.getFontListsForResolution()
    ...
    if not radios[resolution] then
        wfsuite.utils.log("Unsupported resolution: " .. resolution .. ". Using default fonts.", "info")
...
-- until wfsuite.theme.version or box._param_version changes.
function utils.ensureCfg(box, builder)
    local theme_version = (wfsuite and wfsuite.theme and wfsuite.theme.version) or 0
...
function utils.box(x, y, w, h, title, titlepos, titlealign, titlefont, titlespacing, titlecolor, titlepadding, titlepaddingleft, titlepaddingright, titlepaddingtop, titlepaddingbottom, displayValue, unit, font, valuealign, textcolor, valuepadding, valuepaddingleft, valuepaddingright, valuepaddingtop, valuepaddingbottom, bgcolor, image, imagewidth, imageheight, imagealign)
    ...
    if image then
        ...
        if type(image) == "string" and wfsuite and wfsuite.utils and wfsuite.utils.loadImage then
            ...
            if not bitmapPtr then
                bitmapPtr = wfsuite.utils.loadImage(image, nil, fallbackLogo)
...
local function refreshBatteryConfigCache()
    local session = wfsuite and wfsuite.session
...
function utils.isElectricEngine()
    local batteryPrefs = wfsuite and wfsuite.session and wfsuite.session.modelPreferences and wfsuite.session.modelPreferences.battery
...
function utils.updateProgressDialogMessage(statusOverride)
    ...
    local showDebug = wfsuite.preferences and wfsuite.preferences.general and wfsuite.preferences.general.mspstatusdialog
    local mspStatus = statusOverride or (wfsuite.session and wfsuite.session.mspStatusMessage) or nil
