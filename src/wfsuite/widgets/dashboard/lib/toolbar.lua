...
local DEFAULT_TOOLBAR_ITEMS = {
    {
        ...
        enableFunction = function(dashboard, wfsuite)
            if wfsuite.sysIndex['app'] and system.openPage ~= nil then
                return true
            end
            return false
        end,
...
local function resolveToolbarThemeColor(lcd, themeColorKey, fallback)
    if type(themeColorKey) == "number"
        and wfsuite
        and wfsuite.utils
        and wfsuite.utils.ethosVersionAtLeast
        and wfsuite.utils.ethosVersionAtLeast({26, 1, 0})
...
local function isItemEnabled(item, dashboard, wfsuite)
    if not item then return false end
    if type(item.enableFunction) == "function" then
        return item.enableFunction(dashboard, wfsuite) == true
    end
    if item.isConnected == true then
        local sess = wfsuite and wfsuite.session
    ...
    if item.apiVersion and wfsuite and wfsuite.utils and wfsuite.utils.apiVersionCompare then
        local op = item.apiVersionOp or ">="
        if not wfsuite.utils.apiVersionCompare(op, item.apiVersion) then return false end
    end
...
function M.draw(dashboard, wfsuite, lcd, sort, max, FONT_XS, CENTERED, THEME_DEFAULT_COLOR, THEME_DEFAULT_BGCOLOR, THEME_FOCUS_COLOR, THEME_FOCUS_BGCOLOR)
    ...
    for i = 1, slots do
        ...
        if item then
            ...
            local isEnabled = isItemEnabled(item, dashboard, wfsuite)
...
function M.isItemEnabled(item, dashboard, wfsuite)
    return isItemEnabled(item, dashboard, wfsuite)
end
