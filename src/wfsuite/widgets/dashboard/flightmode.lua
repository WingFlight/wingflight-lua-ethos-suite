...
-- `wfsuite.session.rx.values.throttle`, via
...
-- The rotorflight-lua-ethos-suite "Total rewrite" (#2256), ported here,
-- swapped that for the FC's own telemetry-reported throttle_percent sensor
-- (tasks/session.lua), because the RX_MAP fetch (lib/msp_rx_map.lua) hadn't
-- been ported yet -- see lib/msp_handshake.lua's own "Deliberately excludes
-- rxmap" comment. That swap is what's being undone here: a telemetry sensor
-- has to actually be broadcast and resolved over the link, and (self-caught
-- live, on rotorflight-lua-ethos-suite -- same widget code, ported as-is)
-- simply doesn't behave the same as raw stick position once the governor is
-- off -- the dashboard could get stuck showing "postflight" after a rearm
-- because throttle_percent never convincingly crossed THROTTLE_THRESHOLD
-- again. resolveThrottleChannelValue() below restores the original
-- radio-channel read (session.rxMap, fetched once per connect by
-- tasks/session.lua), falling back to the telemetry sensor only if RX_MAP
-- hasn't resolved yet (older FC, or still mid-handshake) rather than never
-- detecting flight.
--
-- isGovernorActive()/widget.governorState is a no-op for this fixed-wing
-- suite -- wfsuite's own tasks/session.lua never fetches a governor config
-- (no such firmware concept for planes), so governorState stays nil and
-- inFlight() always falls through to the throttle check below. Ported
-- unchanged from rotorflight-lua-ethos-suite rather than stripped, since it
-- costs nothing left in and keeps this file diffable against that one --
-- worth revisiting if wfsuite ever wants its own, differently-tuned
-- inFlight() (e.g. airspeed-based rather than throttle-based).
--
...
if package.loaded["wfsuite.widgets.dashboard.flightmode"] then
  return package.loaded["wfsuite.widgets.dashboard.flightmode"]
end
...
package.loaded["wfsuite.widgets.dashboard.flightmode"] = flightmode
return flightmode
