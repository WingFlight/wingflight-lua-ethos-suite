...
local function paint(widget)
  ...
  if paintDashboard(widget, w, h) == false then
    -- engine.paint() already draws the shell (background + box titles)
    -- itself whenever it returns false during cold-start warm-up -- see
    -- its own comment -- so there's nothing placeholder-specific left to
    -- do here; just the ordinary retry-next-tick bookkeeping.
    requestPaint(widget)
...
local function wakeup(widget)
  ...
  -- Matches master's dashboard.lua wakeup(), which does the same poll.
  local now = clock()
