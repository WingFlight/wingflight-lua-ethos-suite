...
local function loadWidget(widget)
    ...
    local file = "SCRIPTS:/" .. wfsuite.config.baseDir .. "/" .. path
...
function toolbox.paint(widget)
    ...
    if not wfsuite.session.toolbox then return end
    ...
    local msg = wfsuite.session.toolbox[toolBoxList[widget.object].object] or "-"
...
function toolbox.wakeup(widget)
    ...
    if not wfsuite.session.toolbox then
        wfsuite.session.toolbox = {}
        return
    end
...
function toolbox.close() wfsuite.session.toolbox = nil end
