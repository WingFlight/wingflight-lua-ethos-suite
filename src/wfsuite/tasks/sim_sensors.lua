...
--   - `flight_mode` (uid 0x5028) has no counterpart in that original at
--     all -- this rebuild's own lib/telemetry_sensors.lua/tasks/session.lua
--     added it (fixed-wing flight-mode tracking has no heli equivalent),
--     so it gets a freshly-picked uid one past that original's own
--     0x5001-0x5027 range instead of a ported value.
--
...
local SENSORS = {
  ...
  osc_limiter      = {uid = 0x5030, name = "Osc Limiter",      unit = nil,                dec = 0,   min = 0,     max = 65536},
}
