...
local session = {
  ...
  fuelPercent = nil,
  governorMode = nil, -- always nil: MSP_GOVERNOR_CONFIG (heli engine-RPM governor mode) no longer
                       -- exists on wingflight-firmware's wire at all; every consumer (dashboard,
                       -- ActiveLook, audio_events.lua's announceGovernor()) already treats nil the
                       -- same as "unknown/inactive", so this is left declared but never populated
                       -- rather than threading a removal through every reader.
  governorState = nil, -- live telemetry sensor value ("governor"), unrelated to the config command
  ...
  -- tasks/flightmode.lua. NOT the same thing as flightModeFlags above
  -- (wingflight-firmware's own autopilot mode telemetry) -- see that
  -- field's own comment. Published on "session.update" so any subscriber
...
-- change five fields in one call, and updateGovernor()/updateFlightMode()
-- run in that same scheduled block too -- and each change used to trigger
-- its own full ~30-field table rebuild + bus dispatch. publish() now just
-- marks `dirty`; wakeup() flushes at most once per tick (see its own end),
-- which collapses N publishes-per-tick into 1 without changing what
-- subscribers see -- they only ever read the latest snapshot, never an
-- intermediate one. This is the source of the Lua-heap sawtooth visible in
-- the "[bgtask mem] lua=" background log.
local dirty = false
...
-- Matches wfsuite.utils.playConnectBeep() in the original suite: play
...
-- Rotorflight FC with a wfsuite build.
...
local function updateFlightMode(protocol)
  if not telemetrySensors then return end
...
-- Recomputes flightmodeState (preflight/inflight/postflight -- NOT the same
-- thing as updateFlightMode() above, see session.flightmodeState's own
-- comment) every tick from whatever session.isArmed/governorState/
-- throttlePercent/connected currently hold (each maintained on its own
-- schedule elsewhere in this file -- same eventual-consistency model
-- widgets/dashboard.lua's own per-tick recompute used before this moved
-- here, not a regression). Named updateFlightStage(), not
-- updateFlightMode(), specifically to avoid colliding with the function
-- above. Unconditional, not gated behind SENSOR_CHECKS_ENABLED, matching
-- updateFlightTimer()'s own placement.
local function updateFlightStage()
...
-- wakeup() tick's updateFlightStage() would recompute from
