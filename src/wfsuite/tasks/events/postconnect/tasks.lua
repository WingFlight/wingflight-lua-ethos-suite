...
local function failTask(task, reason)
    ...
    wfsuite.utils.log("postconnect/" .. tostring(task.name) .. " failed: " .. tostring(reason), "info")
end
...
function tasks.wakeup()
    ...
    if not (wfsuite.session and wfsuite.session.isConnected) then
    ...
    if not lastConnected then
        ...
        resetQueuesAndState()
        wfsuite.utils.log("Postconnect: [started]", "info")
    end
    ...
    if isQueueDone() then
        active = false
        wfsuite.utils.log("Postconnect: [complete]", "info")
        wfsuite.session.postConnectComplete = true
        return
    end
    ...
    if task.startTime and (now - task.startTime) > DEFAULT_TASK_TIMEOUT_SECONDS then
        ...
        if task.attempts <= MAX_RETRIES then
            ...
            wfsuite.utils.log("postconnect/" .. task.name .. " timeout, retry " .. task.attempts .. "/" .. MAX_RETRIES, "info")
            return
        end
    ...
    if module.isComplete and module.isComplete() then
        ...
        task.nextEligibleAt = 0
        releaseTaskModule(task, false)
        wfsuite.utils.log("Completed postconnect/" .. task.name, "debug")
        advanceQueue()
    end
end
