...
function tasks.fire(args)
    ...
    pendingFire = true
    active = true
    wfsuite.utils.log("ondisconnect fired", "debug")
end
...
function tasks.wakeup()
    ...
    wfsuite.utils.log("Connection [lost]. (ondisconnect hook)", "info")
    ...
    while not isQueueDone() do
        ...
        if not task or task.complete or task.failed then
            ...
            if not module or type(module.wakeup) ~= "function" then
                task.failed = true
                wfsuite.utils.log("Failed to load ondisconnect/" .. tostring(task and task.name) .. ": " .. tostring(err or "missing module"), "info")
                queueIndex = (queueIndex or 1) + 1
                goto continue
            end
