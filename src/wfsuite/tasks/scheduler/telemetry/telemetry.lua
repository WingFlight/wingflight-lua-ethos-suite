...
local simSensors = wfsuite.utils.simSensors
...
local function loadSensorMetadata()
    ...
    if not metadataLoader then
        if wfsuite and wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("[telemetry] Failed to load sensor metadata table: " .. tostring(metadataErr), "error")
        end
        return {}
    end
    ...
    if t_type(metadataTable) ~= "table" then
        if wfsuite and wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("[telemetry] Sensor metadata file did not return a table", "error")
        end
        return {}
    end
...
local function loadSourceTable(mode, keepOtherModes)
    ...
    if not sourceTable then
        ...
        if not sourceLoader then
            if wfsuite and wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[telemetry] Failed to load source table: " .. tostring(sourceErr), "error") end
            return nil
        end
...
local function checkCondition(sensorEntry)
    ...
    local sess = wfsuite.session
    if not (sess and sess.apiVersion) then return true end
    local gt, lt = sensorEntry.mspgt, sensorEntry.msplt
    if gt and not wfsuite.utils.apiVersionCompare(">=", gt) then return false end
    if lt and not wfsuite.utils.apiVersionCompare("<=", lt) then return false end
    return true
end
...
function telemetry.active() return (wfsuite.session and wfsuite.session.telemetryState) or false end
...
function telemetry.wakeup()
    ...
    if session and session.telemetryState then
        wfsuite.utils.resolveArmedState()
    end
