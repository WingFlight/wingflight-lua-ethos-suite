...
local function tryLoadImagePath(path)
    ...
    for i = 1, #tryPaths do
        ...
        if not utils.isImageTooLarge(candidate, wfsuite.config.maxModelImageBytes) then
...
function craftimage.wakeup()
    ...
    if not bitmapPtr then bitmapPtr = wfsuite.utils.loadImage(default_image) end
