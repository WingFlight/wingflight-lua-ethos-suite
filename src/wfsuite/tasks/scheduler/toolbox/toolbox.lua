...
local function loadTaskModule(entry)
    ...
    if not chunk then
        wfsuite.utils.log("Error loading toolbox task " .. tostring(entry.path) .. ": " .. tostring(err or "?"), "info")
        return nil
    end

    local module = chunk(config)
    if type(module) ~= "table" or type(module.wakeup) ~= "function" then
        wfsuite.utils.log("Invalid toolbox task module: " .. tostring(entry.path), "info")
        return nil
    end
