...
local function finalizeBatteryConfig(config)
    wfsuite.session.batteryConfig = config
...
local function readSmartFuelConfig(config)
    ...
    local smartFuelAPI = wfsuite.tasks.msp.api.load("SMARTFUEL_CONFIG")
...
function battery.wakeup()
    ...
    if (wfsuite.session.batteryConfig == nil) and mspCallMade == false then
        mspCallMade = true
...
function battery.reset()
    wfsuite.session.batteryConfig = nil
    mspCallMade = false
end

function battery.isComplete() if wfsuite.session.batteryConfig ~= nil then return true end end
