...
local telemetry
local lastMode = wfsuite.flightmode.current or "preflight"
local currentMode = wfsuite.flightmode.current or "preflight"
...
local function logSmartFuelStatus(status, detail)
    ...
    local logger = wfsuite and wfsuite.utils and wfsuite.utils.log
...
local function resetState()
    ...
    telemetry = nil
    currentMode = wfsuite.flightmode.current or "preflight"
...
local function shouldResetForModeChange(previousMode, nextMode)
    ...
    return wfsuite.session and wfsuite.session.isArmed == false
end

local function smartFuelCalc()

    if not telemetry then telemetry = wfsuite.tasks.telemetry end
    ...
    if normalizedBatType ~= nil then
        wfsuite.session.activeBatteryType = normalizedBatType
    end

    if not wfsuite.session.isConnected or not wfsuite.session.batteryConfig then
        resetState()
        return nil
    end

    local bc = wfsuite.session.batteryConfig

    local packCapacity = bc.batteryCapacity
    local activeProfile = wfsuite.session.activeBatteryType
    ...
    if shouldResetForModeChange(lastMode, currentMode) then
        wfsuite.utils.log("Flight mode changed – resetting voltage & fuel state", "info")
    ...
    local isDisarmed = (wfsuite and wfsuite.session and wfsuite.session.isArmed == false)
    local isPreflight = (wfsuite and wfsuite.flightmode and wfsuite.flightmode.current == "preflight")
    ...
    if lastVoltages and #lastVoltages >= 2 and isPreflight and isDisarmed then
        ...
        if voltage > prev + voltageThreshold then
            wfsuite.utils.log("Voltage increased after stabilization – resetting...", "info")
            fuelStartingPercent = nil
            fuelStartingConsumption = nil
            lastFuelPercent = nil
            resetVoltageTracking()
