...
local function applySagCompensation(cellVoltage)
    if wfsuite.flightmode.current ~= "inflight" then return cellVoltage end
...
local function getPackCapacity(bc)
    ...
    if idx ~= nil then wfsuite.session.activeBatteryType = idx end

    local capacity = bc.batteryCapacity
    local active   = wfsuite.session.activeBatteryType
...
local function smartFuelCalc()
    ...
    if lastMode ~= currentMode and currentMode == "preflight" and wfsuite.session.isArmed == false then
        resetFuelState(now)
    ...
    if #lastVoltages >= 2 and currentMode == "preflight" and wfsuite.session.isArmed == false then
    ...
    local isArmed = wfsuite.session.isArmed == true
