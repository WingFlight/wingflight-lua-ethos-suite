--[[
  Copyright (C) 2025 Rotorflight Project
  GPLv3 — https://www.gnu.org/licenses/gpl-3.0.en.html
]] --

local wfsuite = require("wfsuite")

local telemetryconfig = {}
local tonumber = tonumber
local tostring = tostring
local table_concat = table.concat
local log = wfsuite.utils.log

local mspCallMade = false

function telemetryconfig.wakeup()

    if wfsuite.session.apiVersion == nil then return end
    if wfsuite.session.mspBusy then return end
    if wfsuite.tasks.msp.mspQueue:isProcessed() == false then return end

    if (wfsuite.session.telemetryConfig == nil) and (mspCallMade == false) then
        mspCallMade = true
        local API = wfsuite.tasks.msp.api.load("TELEMETRY_CONFIG")
        API.setCompleteHandler(function(self, buf)
            local apiData = API.data()
            local data = apiData.parsed
            local rawBuffer = apiData.buffer

            local slots = {}
            local configBuffer = {}
            for i = 1, 40 do
                local key = "telem_sensor_slot_" .. i
                slots[i] = tonumber(data[key]) or 0
            end
            for i = 1, #rawBuffer do
                configBuffer[i] = rawBuffer[i]
            end

            wfsuite.session.telemetryConfig = slots
            wfsuite.session.telemetryConfigBuffer = configBuffer
            wfsuite.session.crsfTelemetryConfig = {
                mode = tonumber(data["crsf_telemetry_mode"]) or 0,
                linkRate = tonumber(data["crsf_telemetry_link_rate"]) or 0,
                linkRatio = tonumber(data["crsf_telemetry_link_ratio"]) or 0
            }

            local parts = {}
            for i, v in ipairs(slots) do 
                if v ~= 0 then 
                    parts[#parts + 1] = tostring(v) 
                end 
            end
            local slotsStr = table_concat(parts, ",")

            if log then 
                log("Updated telemetry sensors: " .. slotsStr, "info") 
                log("Updated telemetry sensors: " .. tostring(#parts) .. " of " .. tostring(#slots), "connect")
                log(
                    "CRSF telemetry config: mode="
                        .. tostring(wfsuite.session.crsfTelemetryConfig.mode)
                        .. ", rate="
                        .. tostring(wfsuite.session.crsfTelemetryConfig.linkRate)
                        .. ", ratio="
                        .. tostring(wfsuite.session.crsfTelemetryConfig.linkRatio),
                    "info"
                )
            end    
        end)
        API.setErrorHandler(function(self, err)
            log("Failed to read telemetry config via MSP: " .. err, "info")
            mspCallMade = false
        end)
        API.setUUID("sensor-tlmcfg-read")
        API.setTimeout(3000)
        API.read()
    end

end

function telemetryconfig.reset()

    wfsuite.session.telemetryConfig = nil
    wfsuite.session.telemetryConfigBuffer = nil
    wfsuite.session.crsfTelemetryConfig = nil
    mspCallMade = false
end

function telemetryconfig.isComplete() 
    if wfsuite.session.telemetryConfig ~= nil then 
        return true 
    end 
end

return telemetryconfig
