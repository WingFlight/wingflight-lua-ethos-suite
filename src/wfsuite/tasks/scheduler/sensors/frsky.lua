...
local function loadSidLookup()
    ...
    if not lookupLoader then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[frsky] Failed to load SID lookup table: " .. tostring(lookupErr), "error") end
        return {}
    end

    local lookupTable = lookupLoader()
    if type(lookupTable) ~= "table" then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[frsky] SID lookup file did not return a table", "error") end
        return {}
    end
    ...
    if createSensorList[appId] ~= nil then
        ...
        if frsky.createSensorCache[appId] == nil then
            ...
            if frsky.createSensorCache[appId] == nil then
                ...
                frsky.createSensorCache[appId]:module(wfsuite.session.telemetrySensor:module())
...
local function dropSensor(physId, primId, appId, frameValue)
    ...
    if wfsuite.utils.apiVersionCompare(">=", {12, 0, 8}) then return end
        ...
        if apps then
            for _, appId in ipairs(apps) do
                if appId then
                    ...
                    if wfsuite.session.apiVersion ~= nil and wfsuite.utils.apiVersionCompare("<", {12, 0, 8}) then
...
function frsky.wakeup()
    ...
    if not sensorTlm then
        ...
        sensorTlm:module(wfsuite.session.telemetrySensor:module())
    ...
    if not wfsuite.session.telemetryState or not wfsuite.session.telemetrySensor then clearCaches() end
