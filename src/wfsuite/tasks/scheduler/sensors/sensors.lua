...
local function loadSensorModule()
    ...
    if not wfsuite.session.apiVersion then return nil end
...
function sensors.wakeup()
    ...
    if wfsuite.session.batteryConfig == nil then
    ...
    if wfsuite.session.resetSensors and not delayPending then
        delayStartTime = os_clock()
        delayPending = true
        wfsuite.session.resetSensors = false
    ...
    if loadedSensorModule and loadedSensorModule.module.wakeup then
        ...
        if cycleFlip == 0 then
            if wfsuite.session and wfsuite.session.isConnected then
