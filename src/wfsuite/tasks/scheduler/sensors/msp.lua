...
local msp_sensors = {
    DATAFLASH_SUMMARY = {
        ...
        interval_disarmed = 20,   -- disable periodic polling
...
local function isInFlightNow()
    return (wfsuite.flightmode and wfsuite.flightmode.current == "inflight") and true or false
end
...
local function createOrUpdateSensor(appId, fieldMeta, value)
    ...
    local currentModule = wfsuite.session.telemetrySensor and wfsuite.session.telemetrySensor:module()
    ...
    if not sensorCache[appId] and not negativeCache[appId] then
        ...
        if existingSensor then
            ...
            if not (wfsuite.session.telemetryState and wfsuite.session.telemetrySensor) then
            ...
            sensor:appId(appId)
            sensor:physId(0)
            sensor:module(wfsuite.session.telemetrySensor:module())
...
local function updateSessionField(meta, value)
    if not meta.sessionname or type(wfsuite.session) ~= "table" then return end
    local t = wfsuite.session
...
function msp.wakeup()
    ...
    if wfsuite.flightmode and wfsuite.flightmode.current ~= "inflight" then
        if not wfsuite.session.isConnected then return end
        if wfsuite.session.mspBusy then return end
        if wfsuite.tasks and wfsuite.tasks.onconnect and wfsuite.tasks.onconnect.active and wfsuite.tasks.onconnect.active() then return end
    end

    msp.clock = os_clock()

    if firstWakeup then
        log = wfsuite.utils.log
        tasks = wfsuite.tasks
        firstWakeup = false
    end
    ...
    if wfsuite.session.resetMSPSensors then
        sensorCache = {}
        negativeCache = {}
        lastValue = {}
        lastPush = {}
        wfsuite.session.resetMSPSensors = false
    end
    ...
    local isAdmin = wfsuite.tasks.appRunning == true

    local isConnected = wfsuite.session.isConnected == true
