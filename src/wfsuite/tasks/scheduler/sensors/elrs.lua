...
local useRawValue = wfsuite.utils.ethosVersionAtLeast({26, 1, 0})
...
local function loadSidLookup()
    ...
    if not lookupLoader then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[elrs] Failed to load SID lookup table: " .. tostring(lookupErr), "error") end
        return {}
    end

    local lookupTable = lookupLoader()
    if type(lookupTable) ~= "table" then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[elrs] SID lookup file did not return a table", "error") end
        return {}
    end
...
local function rebuildRelevantSidSet()
    ...
    local cfg = wfsuite and wfsuite.session and wfsuite.session.telemetryConfig
    if not cfg then
...
local function createTelemetrySensor(uid, name, unit, dec, value, min, max)
    ...
    if wfsuite.session.telemetryState == false then return end
...
local function setTelemetryValue(uid, subid, instance, value, unit, dec, name, min, max)
    ...
    if sensors['uid'][uid] == nil then
        ...
        if sensors['uid'][uid] == nil then
            if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("Create sensor: " .. tostring(uid), "debug") end
...
local function rebuildActiveSensorsList(force)
    ...
    if not listLoader then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[elrs] Failed to load sensor list: " .. tostring(listErr), "error") end
        sensorsList = {}
    ...
    if type(listFactory) ~= "function" then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[elrs] Sensor list file did not return a factory function", "error") end
        sensorsList = {}
        activeSensorsListSig = sig
        return
    end
    ...
    if type(fullList) ~= "table" then
        if wfsuite.utils and wfsuite.utils.log then wfsuite.utils.log("[elrs] Sensor list factory did not return a table", "error") end
        sensorsList = {}
        activeSensorsListSig = sig
        return
    end
...
function elrs.crossfirePop()
    ...
    if (wfsuite.session.telemetryState == false) then
        local ts = wfsuite.session.telemetrySensor
        if ts then
...
function elrs.wakeup()
    ...
    if not wfsuite.session.isConnected then return end

    rebuildRelevantSidSet()

    if wfsuite.session.telemetryState and wfsuite.session.telemetrySensor then
