...
local function writeStats()
    ...
    if not utils.apiVersionCompare(">=", {22, 0, 0}) then return end
    ...
    local API = wfsuite.tasks.msp.api.load("FLIGHT_STATS")
...
function timer.reset()
    ...
    local timerSession = {}
    wfsuite.session.timer = timerSession
    wfsuite.session.flightCounted = false
    local prefs = wfsuite.session.modelPreferences
...
function timer.save()
    ...
    if prefs then
        ini.setvalue(prefs, "general", "totalflighttime", wfsuite.session.timer.baseLifetime or 0)
        ini.setvalue(prefs, "general", "lastflighttime", wfsuite.session.timer.session or 0)
...
local function finalizeFlightSegment(now)
    local timerSession = wfsuite.session.timer
    local prefs = wfsuite.session.modelPreferences
