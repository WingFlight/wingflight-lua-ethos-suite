...
local function loadAdjWavs()
    ...
    if not chunk then
        wfsuite.utils.log("Error loading adjfunctions wav map " .. ADJ_WAVS_PATH .. ": " .. tostring(err or "?"), "info")
    ...
    if not ok or type(wavs) ~= "table" then
        wfsuite.utils.log("Invalid adjfunctions wav map: " .. tostring(wavs or ADJ_WAVS_PATH), "info")
        adjWavsLoadFailed = true
        return nil
    end
