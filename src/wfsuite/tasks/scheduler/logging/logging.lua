...
local function isDebugEnabled()
    local pref = wfsuite.preferences
...
local function checkLogdirExists(mcu_id)
    ...
    os.mkdir("LOGS:/wfsuite/telemetry/" .. mcu_id)
end
...
local function refreshSessionCaches(session)
    ...
    if cachedMcuId ~= mcu_id then
        cachedMcuId = mcu_id
        cachedBaseDir = "LOGS:/wfsuite/telemetry/" .. mcu_id
...
function logging.wakeup()
    ...
    if not telemetry then
        telemetry = wfsuite.tasks and wfsuite.tasks.telemetry
        return
    end
