...
local function plog(dir, cmd, payload, extra)
    local m = wfsuite.tasks and wfsuite.tasks.msp
...
local function proto() return wfsuite.tasks.msp.protocol end
...
local function pollBudget()
    ...
    local proto   = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.protocol or {}
...
local function _receivedReply(payload)
    ...
    -- V1 CRC check
        ...
        if mspRxCRC ~= rxCRC and versionBits == 0 then
            if wfsuite and wfsuite.session then
                local n = (wfsuite.session.mspCrcErrors or 0) + 1
                wfsuite.session.mspCrcErrors = n
                wfsuite.session.mspStatusMessage = "MSP CRC error (" .. tostring(n) .. ")"
                wfsuite.session.mspStatusUpdatedAt = os.clock()
                local cb = wfsuite.tasks.uiCallbacks
                if cb then
                    if cb.updateProgressDialogMessage then cb.updateProgressDialogMessage(wfsuite.session.mspStatusMessage) end
                    if cb.applyMspStatusToActiveDialogs then cb.applyMspStatusToActiveDialogs(wfsuite.session.mspStatusMessage) end
                end
            end
            return nil
        end
...
local function mspPollReply()
    ...
    local protoCfg = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.protocol or {}
