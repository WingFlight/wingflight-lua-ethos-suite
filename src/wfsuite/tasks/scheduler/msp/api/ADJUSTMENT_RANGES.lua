...
local function parseRead(buf, helper)
    ...
    if slotCount > ADJUSTMENT_RANGE_MAX then
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ADJUSTMENT_RANGES slot count clamped from " .. tostring(slotCount) .. " to " .. tostring(ADJUSTMENT_RANGE_MAX), "info")
        end
        slotCount = ADJUSTMENT_RANGE_MAX
    end

    if (byteCount % ADJUSTMENT_RANGE_BYTES) ~= 0 and wfsuite.utils and wfsuite.utils.log then
        wfsuite.utils.log("ADJUSTMENT_RANGES payload not aligned: " .. tostring(byteCount) .. " bytes", "debug")
    end
