...
local INI_FILE = "SCRIPTS:/" .. wfsuite.config.preferences .. "/models/" .. wfsuite.session.mcu_id .. ".ini"
...
return core.createCustomAPI({
    ...
    customWrite = function(_, state, emitComplete, emitError)
        ...
        for k, v in pairs(state.payloadData) do
            ...
            if wfsuite.session.modelPreferences and wfsuite.session.modelPreferences[INI_SECTION] then
                wfsuite.session.modelPreferences[INI_SECTION][k] = v
                if k == "smartfuel_source" then
                    wfsuite.session.modelPreferences[INI_SECTION].calc_local = v
                end
            end
        end
