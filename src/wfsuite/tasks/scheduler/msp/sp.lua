...
function transport.sportTelemetryPush(sensorId, frameId, dataId, value)
    ...
    if not sensor then
        local activeModule = wfsuite.session.telemetryModuleNumber or 0   -- keep full lookup
...
function transport.mspRead(cmd)
    return wfsuite.tasks.msp.common.mspSendRequest(cmd, {})
end

function transport.mspWrite(cmd, payload)
    return wfsuite.tasks.msp.common.mspSendRequest(cmd, payload)
end
