...
-- Logging toggles
local function LOG_ENABLED_MSP() return wfsuite and wfsuite.preferences and wfsuite.preferences.developer and wfsuite.preferences.developer.logmsp end
local function LOG_ENABLED_MSP_RW() return wfsuite and wfsuite.preferences and wfsuite.preferences.developer and wfsuite.preferences.developer.logmsprw end
...
local function getRwModeSuffix(msg, rwState, ok)
    ...
    local apidata = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.api and wfsuite.tasks.msp.api.apidata
...
local function drainAfterSuccess(self, cmd)
    ...
    local mspCommon = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.common
...
local function LOG_ENABLED_MSP_QUEUE() return wfsuite and wfsuite.preferences and wfsuite.preferences.developer and wfsuite.preferences.developer.logmspQueue end
...
function MspQueueController:processQueue()
    ...
    local now = os_clock()
    local session = wfsuite.session
    local mspTask = wfsuite.tasks.msp
    ...
    if self.mspBusyStart and (now - self.mspBusyStart) > MSP_BUSY_TIMEOUT then
        ...
        --utils.log(" - Unblocking by setting wfsuite.session.mspBusy = false", "info")
        session.mspBusy = false
        self.mspBusyStart = nil
        return
    end
...
function MspQueueController:clear()
    if wfsuite.session then wfsuite.session.mspBusy = false end
    self.mspBusyStart = nil
    ...
    self.uuid = nil
    self.apiname = nil
    if wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.common then
        wfsuite.tasks.msp.common.mspClearTxBuf()
    end
end
...
function MspQueueController:add(message)
    if not wfsuite.session.telemetryState then
    ...
    local pageScript = wfsuite.tasks.lastScript
