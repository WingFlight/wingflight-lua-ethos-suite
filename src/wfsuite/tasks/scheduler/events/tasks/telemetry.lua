...
local FLIGHT_MODE_PRIORITY = {
    ...
    {bit = 7, file = "manual.wav"},       -- PASSTHROUGH_MODE_BIT
...
local function resetSmartfuelSensor()
    local sensorTask = wfsuite.tasks and wfsuite.tasks.sensors
...
local eventTable = {
    {
        ...
        window = 5,
        event = function(value, interval, window)
            if wfsuite.flightmode.current ~= "inflight" then
...
function telemetry.wakeup()
    ...
    now = os_clock()
    local eventPrefs = wfsuite.preferences.events or {}
    local tlmTask = wfsuite.tasks.telemetry
