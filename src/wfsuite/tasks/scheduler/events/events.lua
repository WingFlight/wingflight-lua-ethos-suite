...
for _, name in ipairs(taskNames) do
    events[name] = assert(loadfile("tasks/scheduler/events/tasks/" .. name .. ".lua"))(wfsuite.config)
...
function events.wakeup()
    ...
    if wfsuite.session.postConnectComplete and wfsuite.session.telemetryState then
