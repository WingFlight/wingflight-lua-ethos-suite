...
return {
  ...
  -- "link" (session.linkQuality) had no CRSF entry at all until now -- a
  -- pre-existing gap, not something the rssi/vfr addition above introduced:
  -- S.Port's own candidate table (see telemetry_sensors_sport.lua) already
  -- had one, CRSF's didn't, so linkQuality silently stayed nil for every
  -- CRSF/ELRS pilot. Unlike rssi/vfr above, "link" does NOT try subId=2
  -- (the dedicated LQ field) first -- matches the pre-extraction
  -- widgets/dashboard/context.lua's own LIVE_SENSOR_CANDIDATES table
  -- exactly, subIdStart/subIdEnd range first, falling back to a plain
  -- named-sensor lookup ("Rx RSSI1", a string candidate -- system.getSource()
  -- accepts either shape, same as this module's own getSource() loop
  -- already assumes).
  link = {
    {crsfId = 0x14, subIdStart = 0, subIdEnd = 1},
    "Rx RSSI1",
