-- ELRS TX-module <-> Wingflight CRSF-telemetry-config probe/sync.
--
-- Ported from rotorflight-lua-ethos-suite's master branch
-- (app/modules/diagnostics/tools/elrslink_task.lua), by way of that
-- suite's own rfsuite-full-rewrite branch (src/rfsuite/lib/
-- elrslink_task.lua). Talks to the ELRS TX module directly over CRSF's
-- *extended* parameter protocol (device ping/info, parameter read/write --
-- frame types 0x28/0x29/0x2B/0x2C/0x2D), which is a completely different
-- frame family from the MSP req/resp and custom-telemetry (0x88) frames
-- tasks/msp/transport_crsf.lua handles -- so, like the original, this owns
-- its own crsf.getSensor() handle rather than going through that transport
-- (which is private to tasks/msp/ anyway, and doesn't expose these frame
-- types).
--
...
--     the original.
--   - No shared session table access -- subscribes to "session.update"
...
-- unsubscribes.
if package.loaded["wfsuite.lib.elrslink_task"] then
...
local SYNC_MODE_FC_TO_ELRS = 1
local SYNC_MODE_ELRS_TO_FC = 2
...
local T = {
  actionFcToElrs = "@i18n(app.modules.elrs_telemetry.action_wf_to_elrs)@",
  actionElrsToFc = "@i18n(app.modules.elrs_telemetry.action_elrs_to_wf)@",
  ...
  elrsMatchesFc = "@i18n(app.modules.elrs_telemetry.status_elrs_matches_wf)@",
  elrsProbeComplete = "@i18n(app.modules.elrs_telemetry.status_elrs_probe_complete)@",
  fcMatchesElrs = "@i18n(app.modules.elrs_telemetry.status_wf_matches_elrs)@",
  writingFc = "@i18n(app.modules.elrs_telemetry.status_writing_wf)@",
  savingFc = "@i18n(app.modules.elrs_telemetry.status_saving_wf)@",
  fcUpdated = "@i18n(app.modules.elrs_telemetry.status_wf_updated)@",
  fcSaveFailed = "@i18n(app.modules.elrs_telemetry.status_wf_save_failed)@",
  fcWriteFailed = "@i18n(app.modules.elrs_telemetry.status_wf_write_failed)@",
  ...
  waitingFcConfig = "@i18n(app.modules.elrs_telemetry.status_waiting_wf_config)@",
  fcConfigNotReady = "@i18n(app.modules.elrs_telemetry.status_wf_config_not_ready)@",
...
local function syncModeLabel(mode)
  if mode == SYNC_MODE_FC_TO_ELRS then return T.actionFcToElrs end
  if mode == SYNC_MODE_ELRS_TO_FC then return T.actionElrsToFc end
  return T.actionProbeOnly
end

local function normalizeSyncMode(mode)
  if mode == SYNC_MODE_FC_TO_ELRS or mode == SYNC_MODE_ELRS_TO_FC then
    return mode
  end
...
local function syncFcToElrs(fc)
  ...
  if pendingWriteCount > 0 then
    ...
    log("Syncing ELRS module to Wingflight: " .. table.concat(actions, ", "))
  ...
  elseif rateTargetIndex == nil then
    log("ELRS sync could not map Wingflight rate " .. tostring(fc.linkRate) .. "Hz to a module option")
  end

  if ratioField == nil then
    log("ELRS sync could not find the module telemetry-ratio field")
  elseif ratioTargetIndex == nil then
    log("ELRS sync could not map Wingflight ratio 1:" .. tostring(fc.linkRatio) .. " to a module option")
  end
  ...
  if rateField and ratioField and rateTargetIndex == rateField.selectedIndex and ratioTargetIndex == ratioField.selectedIndex then
    log("ELRS module already follows Wingflight")
    setStatus(T.elrsMatchesFc)
  else
...
local function syncElrsToFc(fc, moduleRate, moduleRateText, moduleRatioText, ratioKind, effectiveRatio)
  ...
  if type(fcConfigData) ~= "table" then
    log("ELRS sync could not update Wingflight because the telemetry config was not read")
    completeTask()
    return
  end

  if fc.linkRate == moduleRate and fc.linkRatio == effectiveRatio then
    log("Wingflight already follows ELRS numerically")
    setStatus(T.fcMatchesElrs)
    completeTask()
    return
  end
...
local function finalize()
  ...
  if fcConfig then
    log("Wingflight CRSF telemetry: mode=" .. telemetryModeLabel(fcConfig.mode) .. ", rate=" .. tostring(fcConfig.linkRate) .. ", ratio=1:" .. tostring(fcConfig.linkRatio))
  end
  ...
  if syncMode == SYNC_MODE_ELRS_TO_FC then
    syncElrsToFc(fcConfig, moduleRate, moduleRateLabel, moduleRatioLabel, ratioKind, effectiveRatio)
    return
  end

  syncFcToElrs(fcConfig)
end
...
-- start(), since the module itself could have changed), the Wingflight
...
-- Fetches Wingflight's own CRSF telemetry config (mode/rate/ratio) once
...
-- warm the Wingflight summary before a probe/sync is ever started --
...
-- {mode, linkRate, linkRatio} from the last successful Wingflight
...
elrslink.MODE_FC_TO_ELRS = SYNC_MODE_FC_TO_ELRS
elrslink.MODE_ELRS_TO_FC = SYNC_MODE_ELRS_TO_FC

resetState()
taskComplete = true
setStatus(T.idle)

package.loaded["wfsuite.lib.elrslink_task"] = elrslink
return elrslink
