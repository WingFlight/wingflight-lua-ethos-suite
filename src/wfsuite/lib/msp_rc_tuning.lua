...
--
-- Field order/types confirmed directly against wingflight-firmware's
-- actual wire serializer (src/main/msp/msp.c's MSP_RC_TUNING/
-- MSP_SET_RC_TUNING cases): field count/order is unchanged from the
-- rotorflight-based guess this replaced -- grouped axis-by-axis (all 5
-- fields of axis 1, then axis 2, etc.), no "wire order differs from
...
-- Actual/Quick/Wingflight all reuse the same three wire fields per axis,
...
-- printing" section).
if package.loaded["wfsuite.lib.msp_rc_tuning"] then
  return package.loaded["wfsuite.lib.msp_rc_tuning"]
end
...
package.loaded["wfsuite.lib.msp_rc_tuning"] = msp_rc_tuning
