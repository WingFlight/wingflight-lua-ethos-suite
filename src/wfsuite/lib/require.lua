...
-- theoretical (on rotorflight-lua-ethos-suite, same repo shape): lib/bus.lua
-- and lib/mspcodec.lua were each loadfile()'d 12 times, lib/settings_store.lua
-- 10 times, in well under a second -- half of all loadFile() calls logged
-- in that window were reloading a file the boot had already loaded.
--
...
-- package.loaded table (under the same "wfsuite.<path>" keys) every
...
if package.loaded["wfsuite.lib.require"] then
  return package.loaded["wfsuite.lib.require"]
end

local NAMESPACE = "wfsuite"

-- "lib/bus.lua" -> "wfsuite.lib.bus", matching the key every module
...
package.loaded["wfsuite.lib.require"] = requireModule
