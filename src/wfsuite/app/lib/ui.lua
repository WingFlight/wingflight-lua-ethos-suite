...
local arg = {...}
local config = arg[1]
local preferences = wfsuite.preferences
...
local function getApiCore()
    ...
    apiCore = assert(loadfile("SCRIPTS:/" .. wfsuite.config.baseDir .. "/tasks/scheduler/msp/api_core.lua"))()
    return apiCore
end
...
local function progressDialogWakeup()
    ...
    if not app.triggers.closeProgressLoader then
        ...
        if app.dialogs.progressCounter > 50 and session.apiVersion and not utils.stringInArray(wfsuite.config.supportedMspApiVersion, apiV) then print("No API version yet") end
    ...
    if app.dialogs.progressWatchDog and tasks.msp and (osClock() - app.dialogs.progressWatchDog) > tonumber(tasks.msp.protocol.pageReqTimeout) and app.dialogs.progressDisplay == true and app.dialogs.progressTimedOut ~= true then
        ...
        if app.pageState == app.pageStatus.rebooting or wfsuite.session.rebootInProgress or (session and session.resetMSP) then
            app.dialogs.progressCounter = 0
            app.dialogs.progressSpeed = nil
            app.dialogs.progressDisplay = false
            app.dialogs.progressWatchDog = nil
...
local function saveDialogWakeup()
    ...
    if (watchdogExceeded or progressExceeded) and app.dialogs.saveDisplay == true and app.dialogs.saveTimedOut ~= true then
        ...
        if app.pageState == app.pageStatus.rebooting or wfsuite.session.rebootInProgress then
            app.dialogs.saveProgressCounter = 0
            app.dialogs.saveDisplay = false
            app.dialogs.saveWatchDog = nil
            app.triggers.isSaving = false
...
function ui.rebootFc(sourcePage)
    ...
    wfsuite.session.rebootInProgress = true
...
function ui.adminStatsOverlay()
    ...
    local luaRamKB = (wfsuite.performance and wfsuite.performance.luaRamKB) or 0
...
function ui.wakeupAdminStatsOverlay()
    ...
    local perf = wfsuite.performance or EMPTY
