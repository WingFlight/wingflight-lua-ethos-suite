...
local function scheduleIn(delaySeconds, fn)
    ...
    local scheduler = wfsuite.tasks and wfsuite.tasks.callback
    ...
    return function(_, onComplete)
        ...
        local function finish()
            ...
            if wfsuite.utils and wfsuite.utils.log then
                wfsuite.utils.log("ESC 4WIF post-save cycle complete", "info")
            end
            if complete then complete() end
        end

        local target = normalizeEscTarget(wfsuite.session and wfsuite.session.esc4WayTarget, ESC)
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ESC 4WIF post-save cycle: " .. tostring(resetTarget) .. " -> " .. tostring(target), "info")
        end

        local function restoreTarget()
            writeEsc4WayTarget(target, writeOpts, function(success, err, attempts)
                if (not success) and wfsuite.utils and wfsuite.utils.log then
                    wfsuite.utils.log("ESC 4WIF post-save restore failed: " .. tostring(err) .. " (attempts=" .. tostring(attempts) .. ")", "info")
                end
                if not success then
                ...
                scheduleIn(flushReadDelay, function()
                    readEscApiOnce(flushReadApi, readOpts, function(readOk, readErr, readAttempts)
                        if (not readOk) and wfsuite.utils and wfsuite.utils.log then
                            wfsuite.utils.log(
        ...
        local function resetTargetAfterSettle()
            writeEsc4WayTarget(resetTarget, writeOpts, function(success, err, attempts)
                if (not success) and wfsuite.utils and wfsuite.utils.log then
                    wfsuite.utils.log("ESC 4WIF post-save reset failed: " .. tostring(err) .. " (attempts=" .. tostring(attempts) .. ")", "info")
                end
...
local function openLocalProgressDialog(opts)
    local useWait = wfsuite.utils and wfsuite.utils.ethosVersionAtLeast and
        wfsuite.utils.ethosVersionAtLeast({26, 1, 0}) and form.openWaitDialog
    if useWait then
        opts.progress = true
...
function escToolsPage.createIsolatedSaveMenuHandler(folder, escConfig)
        ...
        saveState.dialog = openLocalProgressDialog({
            ...
            wakeup = function()
                ...
                elseif isSaveFailed() then
                    if wfsuite.utils and wfsuite.utils.log then
                        wfsuite.utils.log("ESC isolated save failed", "info")
                    end
                ...
                if (os.clock() - saveState.startedAt) > timeoutSeconds then
                    if wfsuite.utils and wfsuite.utils.log then
                        wfsuite.utils.log("ESC isolated save timeout", "info")
                    end
                    dialog:message("@i18n(app.error_timed_out)@")
                    dialog:closeAllowed(true)
                    dialog:value(100)
    ...
    local function onSaveMenu(page)
        ...
        local targetPage = page or (wfsuite.app and wfsuite.app.Page)
        if not targetPage then return true end
        if targetPage.canSave and targetPage.canSave(targetPage) ~= true then return true end

        local prefs = wfsuite.preferences and wfsuite.preferences.general
