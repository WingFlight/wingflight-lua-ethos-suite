...
local function readModeRangesExtra()
    local API = wfsuite.tasks.msp.api.loadPage("MODE_RANGES_EXTRA")
    if not API then
...
local function readModeRanges()
    local API = wfsuite.tasks.msp.api.loadPage("MODE_RANGES")
    if not API then
...
local function readBoxNames()
    local API = wfsuite.tasks.msp.api.loadPage("BOXNAMES")
    if not API then
...
local function readBoxIds()
    local API = wfsuite.tasks.msp.api.loadPage("BOXIDS")
    if not API then
...
local function startLoad()
    ...
    state.needsRender = true
    local page = wfsuite.app and wfsuite.app.Page or nil
    local speed = page and tonumber(page.loaderspeed) or MODULE_LOADER_SPEED
    wfsuite.app.ui.progressDisplay("@i18n(app.modules.modes.name)@", "@i18n(app.modules.modes.loading_config)@", speed)
    readBoxIds()
end
...
local function queueSetModeRange(slotIndex, done, failed)
    ...
    local API = wfsuite.tasks.msp.api.loadPage("SET_MODE_RANGE")
    if not API then
...
local function saveAllRanges()
    state.saving = true
    state.saveError = nil
    wfsuite.app.ui.progressDisplay("@i18n(app.modules.modes.name)@", "@i18n(app.modules.modes.saving_config)@")
