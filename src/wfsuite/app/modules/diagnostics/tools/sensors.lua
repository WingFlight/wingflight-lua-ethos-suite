...
local function rebootFC()
    ...
    local RAPI = tasks.msp.api.loadPage("REBOOT")
...
local function runRepair(data)
    ...
    local WRITEAPI = tasks.msp.api.loadPage("TELEMETRY_CONFIG")
...
local function wakeup()
    ...
    if app.formNavigationFields['tool'] then
        if session and session.apiVersion and rfutils.apiVersionCompare("<", {22, 0, 0}) then
