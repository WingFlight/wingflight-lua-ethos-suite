...
local function syncRotorflightToElrs(fcConfig)
    ...
    if pendingWriteCount > 0 then
        ...
        if ratioField == nil then
            wfsuite.utils.log("ELRS sync could not find the module telemetry-ratio field", "info")
        ...
        if rateField == nil then
            wfsuite.utils.log("ELRS sync could not find the module packet-rate field", "info")
    ...
    if rateField and ratioField and rateTargetIndex == rateField.selectedIndex and ratioTargetIndex == ratioField.selectedIndex then
        wfsuite.utils.log("ELRS module already follows Rotorflight", "info")
...
local function syncElrsToRotorflight(fcConfig, moduleRate, moduleRateText, moduleRatioText, ratioKind, effectiveRatio)
    ...
    if type(sourceBuffer) ~= "table" or #sourceBuffer < 12 then
        wfsuite.utils.log("ELRS sync could not update Rotorflight because the telemetry-config buffer was unavailable", "info")
        completeTask()
        return
    end
    ...
    if fcConfig.linkRate == moduleRate and fcConfig.linkRatio == effectiveRatio then
        if ratioKind == "race" then
            ...
            wfsuite.utils.log("Rotorflight already follows ELRS numerically", "info")
            setStatus(T.rfMatchesElrs)
        end
        completeTask()
        return
    ...
    local api = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.api and wfsuite.tasks.msp.api.load
        and wfsuite.tasks.msp.api.loadPage(TELEMETRY_CONFIG_API_NAME)

    if not writeBuffer or not api then
        wfsuite.utils.log("ELRS sync could not open the Rotorflight telemetry configuration for writing", "info")
        completeTask()
        return
    end
    ...
    api.setCompleteHandler(function()
        ...
        local EAPI = wfsuite.tasks.msp.api.loadPage("EEPROM_WRITE")
        EAPI.setUUID("eeprom.elrslink.diagnostics")
        EAPI.setCompleteHandler(function()
            wfsuite.utils.log("Saved Rotorflight telemetry sync to EEPROM", "info")
            setStatus(T.rotorflightUpdated)
            completeTask()
        end)
        EAPI.setErrorHandler(function()
            wfsuite.utils.log("EEPROM write failed after ELRS telemetry sync", "info")
        ...
        if not ok then
            wfsuite.utils.log("EEPROM enqueue rejected after ELRS telemetry sync (" .. tostring(reason) .. ")", "info")
            setStatus(T.rotorflightSaveFailed)
            completeTask()
        end
    end)
    api.setErrorHandler(function(_, reason)
        clearApiEntry(TELEMETRY_CONFIG_API_NAME)
        wfsuite.utils.log("Failed to sync Rotorflight telemetry from ELRS (" .. tostring(reason) .. ")", "info")
    ...
    if not ok then
        clearApiEntry(TELEMETRY_CONFIG_API_NAME)
        wfsuite.utils.log("Rotorflight telemetry sync write was rejected (" .. tostring(reason) .. ")", "info")
...
local function finalize()
    ...
    if syncMode == SYNC_MODE_OFF then
        wfsuite.utils.log("ELRS telemetry sync is Off; leaving both sides unchanged", "info")
        setStatus(T.probeComplete)
        completeTask()
        return
    end

    wfsuite.utils.log("ELRS telemetry sync mode: " .. syncModeLabel(syncMode), "debug")
...
function elrslink.wakeup()
    ...
    if type(session.crsfTelemetryConfig) ~= "table" then
        ...
        elseif (now - configWaitStartedAt) >= DISCOVERY_TIMEOUT_SECONDS then
            wfsuite.utils.log("Skipping ELRS link probe because CRSF telemetry config was not ready", "info")
    ...
    if state == "idle" then
        ...
        state = "ping"
        nextActionAt = 0
        wfsuite.utils.log("Starting ELRS link probe", "debug")
    ...
    if state == "ping" and probeStartedAt > 0 and (now - probeStartedAt) >= DISCOVERY_TIMEOUT_SECONDS then
        wfsuite.utils.log("No ELRS TX module responded to the CRSF parameter probe", "info")
    ...
    if state == "read" and probeStartedAt > 0 and (now - probeStartedAt) >= READ_TIMEOUT_MAX_SECONDS then
        wfsuite.utils.log("ELRS link probe timed out while reading module parameters", "info")
    ...
    if not crsfSensor then
        wfsuite.utils.log("CRSF sensor unavailable for ELRS link probe", "info")
    ...
    if state == "write" then
        ...
        if pendingWriteIndex > pendingWriteCount then
            if action.fieldKind == "rate" then
                wfsuite.utils.log("ELRS sync requested a packet-rate change; the link may reconnect briefly", "info")
            end
...
function elrslink.start(mode)
    ...
    if wfsuite.utils and wfsuite.utils.inFlight and wfsuite.utils.inFlight() then
