...
local function openPage(opts)
    ...
    addStatusLine("@i18n(app.modules.fblstatus.cpu_load)@", string.format("%.1f%%", wfsuite.performance.cpuload or 0))

    addStatusLine("@i18n(app.modules.msp_speed.memory_free)@", string.format("%.1f kB", wfsuite.performance.freeram or 0))
...
local function wakeup()
    ...
    do
        ...
        if field then field:value(string.format("%.1f%%", wfsuite.performance.cpuload or 0)) end
    end

    do
        local field = app.formFields and app.formFields[IDX_FREERAM]
        if field then field:value(string.format("%.1f kB", rfutils.round(wfsuite.performance.freeram or 0, 1))) end
    end
