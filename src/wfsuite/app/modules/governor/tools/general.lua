...
local apidata = {
    ...
    formdata = {
        ...
        fields = {
            ...
            [FIELDS["GOVERNOR_IDLE_THROTTLE"]] = {t = "@i18n(app.modules.profile_governor.idle_throttle)@", mspapi = 1, apikey = "governor_idle_throttle", xenablefunction = function() return (wfsuite.session.governorMode >= 1) end,}, 
            [FIELDS["GOVERNOR_AUTO_THROTTLE"]] = {t = "@i18n(app.modules.profile_governor.auto_throttle)@", mspapi = 1, apikey = "governor_auto_throttle", xenablefunction = function() return (wfsuite.session.governorMode >= 1) end},
...
local function postSave(self)
    ...
    if wfsuite.session.governorMode ~= wfsuite.app.Page.apidata.formdata.fields[1].value then
        wfsuite.session.governorMode = wfsuite.app.Page.apidata.formdata.fields[1].value
        wfsuite.utils.log("Governor mode: " .. wfsuite.session.governorMode, "info")
    end
    return payload
end
...
local function wakeup()
    ...
    local governorModeField = wfsuite.app.Page.apidata.formdata.fields[FIELDS["GOVERNOR_MODE"]]
    local governorMode = tonumber(governorModeField and governorModeField.value)
    if governorMode == nil then
        governorMode = tonumber(wfsuite.session.governorMode)
    end
    if governorMode == nil then
        return
    end
