...
local STATIC_CACHE_HOST = wfsuite.app or wfsuite
...
local function clearTelemetryStaticCache()
    ...
    if wfsuite and wfsuite.app then
        wfsuite.app._telemetryStaticCache = nil
    end
    if wfsuite then
        wfsuite._telemetryStaticCache = nil
    end
...
local function isLinkReady()
    local liveSession = wfsuite.session
...
local function refreshSaveState()
    if wfsuite.app and wfsuite.app.ui and wfsuite.app.ui.setPageDirty then
        wfsuite.app.ui.setPageDirty(hasConfigChanges())
    end
end

local function setFormFieldsEnabled(enabled)
    if not (wfsuite.app and wfsuite.app.formFields) then return end
    for _, field in pairs(wfsuite.app.formFields) do
...
local function resetConfigRuntimeState()
    ...
    if wfsuite.app and wfsuite.app.Page then
        wfsuite.app.Page.configLoaded = false
    end
...
local function openPage(opts)
    ...
    if wfsuite.utils.apiVersionCompare("<", {12, 0, 8}) then
        ...
        wfsuite.app.formNavigationFields["save"]:enable(false)
        wfsuite.app.formNavigationFields["reload"]:enable(false)
        ...
        if group and group.ids and #group.ids > 0 then
            ...
            for _, id in ipairs(group.ids) do
                ...
                if sensor then
                    ...
                    wfsuite.app.formFields[formFieldCount] = form.addBooleanField(line, nil, function() return config[sensorId] or false end, function(val)
                        ...
                        if val == true and NOT_AT_SAME_TIME[sensorId] then
                            ...
                            for _, conflictId in ipairs(NOT_AT_SAME_TIME[sensorId]) do
                                ...
                                if wfsuite.app.formFields[conflictId] then wfsuite.app.formFields[conflictId]:enable(false) end
                            end
                        elseif val == false and NOT_AT_SAME_TIME[sensorId] then

                            for _, conflictId in ipairs(NOT_AT_SAME_TIME[sensorId]) do
                                if wfsuite.app.formFields[conflictId] then wfsuite.app.formFields[conflictId]:enable(true) end
                    ...
                    wfsuite.app.formFields[formFieldCount]:enable(false)
                end
            end
        end
    end
...
local function applySettings()
    local epoch = lifecycleEpoch
    local EAPI = trackApi(wfsuite.tasks.msp.api.loadPage("EEPROM_WRITE"))
    EAPI.setUUID("telemetry-eeprom")
    EAPI.setCompleteHandler(function(self)
        if not isLifecycleActive(epoch) then return end
        wfsuite.utils.log("Writing to EEPROM", "info")
        rebootFC()
    end)
    EAPI.write()
...
local function applyDefaultSensors()
    local sensorListFromApi = getDefaultSensors(wfsuite.tasks.telemetry.listSensors() or {})
    local defaultSet = {}
...
local function wakeup()
    ...
    elseif lastLinkReady ~= linkReady then
        if not linkReady then
            resetConfigRuntimeState()
        elseif wfsuite.app and wfsuite.app.Page then
            wfsuite.app.Page.configLoaded = false
            configLoading = false
        end
    end
    ...
    if linkReady and not wfsuite.app.Page.configLoaded and not configLoading then
        configLoading = true
        local epoch = lifecycleEpoch

        -- first load the feature config 
        local FAPI = trackApi(wfsuite.tasks.msp.api.loadPage("FEATURE_CONFIG"))
        FAPI.setCompleteHandler(function(self, buf)
                ...
                wfsuite.utils.log("Feature config loaded", "info")
        end)
        FAPI.setUUID("telemetry-feature-read")
        FAPI.read()

        -- now load the telemetry config
        local API = trackApi(wfsuite.tasks.msp.api.loadPage("TELEMETRY_CONFIG"))
        API.setCompleteHandler(function(self, buf)
            if not isLifecycleActive(epoch) then return end
            if wfsuite.app.Page then
                    ...
                    for key, value in pairs(data.parsed) do
                        if string.match(key, "^telem_sensor_slot_%d+$") then
                            ...
                            if sensorId and sensorId ~= 0 then
                                wfsuite.app.Page.config[sensorId] = true
                            end
                        end
                    end
                end
                ...
                refreshSaveState()
                wfsuite.app.Page.configLoaded = true
            end

            configLoading = false
            wfsuite.utils.log("Telemetry config loaded", "info")
            wfsuite.app.triggers.closeProgressLoader = true
        end)
        API.setErrorHandler(function()
            ...
            wfsuite.utils.log("Telemetry config load failed", "error")
            wfsuite.app.triggers.closeProgressLoader = true
        end)
    ...
    if triggerSave == true then
        wfsuite.app.ui.progressDisplaySave("@i18n(app.modules.profile_select.save_settings)@")
        ...
        if FEATURE_ENABLED_BITMAP ~= nil then
            ...
            if not telemetryEnabled then
                wfsuite.utils.log("Telemetry feature disabled – enabling", "info")
        ...
        local epoch = lifecycleEpoch
        local WRITEAPI = trackApi(wfsuite.tasks.msp.api.loadPage("TELEMETRY_CONFIG"))
        WRITEAPI.setUUID("telemetry-config-write")
        WRITEAPI.setCompleteHandler(function(self, buf)
            if not isLifecycleActive(epoch) then return end
            wfsuite.utils.log("Telemetry config written, now writing to EEPROM", "info")
            snapshotConfig(config, SAVED_CONFIG)
            refreshSaveState()
            applySettings()
        end)
        WRITEAPI.setErrorHandler(function(self, buf)
            if not isLifecycleActive(epoch) then return end
            wfsuite.utils.log("Write to fbl failed.", "info")
        end)
        ...
        wfsuite.session = wfsuite.session or {}
        wfsuite.session.telemetryConfig = appliedSensors

        wfsuite.utils.log("Applied telemetry sensors: " .. table.concat(appliedSensors, ", "), "info")
    ...
    if setDefaultSensors == true and wfsuite.app.Page.configLoaded then
        local changed = applyDefaultSensors()
        if changed and wfsuite.app and wfsuite.app.formNavigationFields and wfsuite.app.formNavigationFields["save"] and wfsuite.app.formNavigationFields["save"].enable then
            wfsuite.app.formNavigationFields["save"]:enable(true)
    ...
    if pendingSaveStateRefresh > 0 then
        ...
        refreshSaveState()
        if hasConfigChanges() and wfsuite.app and wfsuite.app.formNavigationFields and wfsuite.app.formNavigationFields["save"] and wfsuite.app.formNavigationFields["save"].enable then
            wfsuite.app.formNavigationFields["save"]:enable(true)
        end
    end
end
...
local function close()
    ...
    if wfsuite.app and wfsuite.app.triggers then
        wfsuite.app.triggers.closeProgressLoader = true
        wfsuite.app.triggers.closeProgressLoaderNoisProcessed = true
