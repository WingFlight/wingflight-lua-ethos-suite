...
local function makePage(modeKey, modeLabel)
    ...
    local function openPage(opts)
        ...
        wfsuite.app.ui.fieldHeader("@i18n(app.modules.settings.name)@" .. " / ActiveLook / " .. modeLabel)
        ...
        local layoutPos = {x = dropX, y = wfsuite.app.radio.linePaddingTop, w = dropW, h = wfsuite.app.radio.navbuttonHeight}
        wfsuite.app.formFields[fieldIdx] = form.addChoiceField(layoutLine, layoutPos, common.LAYOUT_CHOICES, function()
        ...
        local previewPos = {x = previewTextX, y = wfsuite.app.radio.linePaddingTop, w = previewTextW, h = wfsuite.app.radio.navbuttonHeight}
        previewFields[1] = form.addStaticText(layoutLine, previewPos, "")

        wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
        local previewLine = form.addLine("")
        wfsuite.app.formLines[wfsuite.app.formLineCnt] = previewLine
        previewFields[2] = form.addStaticText(previewLine, previewPos, "")

        for i = 1, 4 do
            local key = modeKey .. "_" .. i
            local line, idx = addFieldLine("Slot " .. i)
            local slotPos = {x = fieldX, y = wfsuite.app.radio.linePaddingTop, w = fieldW, h = wfsuite.app.radio.navbuttonHeight}
            ...
            wfsuite.app.formFields[idx] = slotFields[i]
        end
