...
local function generateThemeList()
    ...
    settings = wfsuite.preferences.dashboard or {}

    if wfsuite.session.modelPreferences and type(wfsuite.session.modelPreferences.dashboard) == "table" then
        settings_model = wfsuite.session.modelPreferences.dashboard
    else
        settings_model = {}
    end
...
local function modelThemeFieldsEnabled()
    return (wfsuite.session.isConnected and wfsuite.session.mcu_id) and true or false
end

local function enableField(fieldId, enabled)
    local field = fieldId and wfsuite.app.formFields[fieldId] or nil
...
local function openPage(opts)
    ...
    wfsuite.app.ui.fieldHeader("@i18n(app.modules.settings.name)@" .. " / " .. "@i18n(app.modules.settings.dashboard)@" .. " / " .. "@i18n(app.modules.settings.dashboard_theme)@")
    ...
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = global_panel:addLine("@i18n(app.modules.settings.dashboard_theme_use_same)@")
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = global_panel:addLine("@i18n(app.modules.settings.dashboard_theme_preflight)@")

    wfsuite.app.formFields[formFieldCount] = form.addChoiceField(wfsuite.app.formLines[wfsuite.app.formLineCnt], nil, formattedThemes, function()
        if wfsuite.preferences and wfsuite.preferences.dashboard then
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = global_panel:addLine("@i18n(app.modules.settings.dashboard_theme_inflight)@")
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = global_panel:addLine("@i18n(app.modules.settings.dashboard_theme_postflight)@")
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = model_panel:addLine("@i18n(app.modules.settings.dashboard_theme_use_same)@")
    wfsuite.app.formFields[formFieldCount] = form.addBooleanField(wfsuite.app.formLines[wfsuite.app.formLineCnt], nil, function()
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = model_panel:addLine("@i18n(app.modules.settings.dashboard_theme_preflight)@")

    wfsuite.app.formFields[formFieldCount] = form.addChoiceField(wfsuite.app.formLines[wfsuite.app.formLineCnt], nil, formattedThemesModel, function()
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = model_panel:addLine("@i18n(app.modules.settings.dashboard_theme_inflight)@")
    ...
    formFieldCount = formFieldCount + 1
    wfsuite.app.formLineCnt = wfsuite.app.formLineCnt + 1
    wfsuite.app.formLines[wfsuite.app.formLineCnt] = model_panel:addLine("@i18n(app.modules.settings.dashboard_theme_postflight)@")
...
local function onSaveMenu()
    ...
    local function doSave()
        ...
        for key, value in pairs(settings) do wfsuite.preferences.dashboard[key] = value end
        wfsuite.ini.save_ini_file("SCRIPTS:/" .. wfsuite.config.preferences .. "/preferences.ini", wfsuite.preferences)

        if wfsuite.session.isConnected and wfsuite.session.mcu_id and wfsuite.session.modelPreferencesFile then
            wfsuite.session.modelPreferences.dashboard = wfsuite.session.modelPreferences.dashboard or {}
            for key, value in pairs(settings_model) do wfsuite.session.modelPreferences.dashboard[key] = value end
            wfsuite.ini.save_ini_file(wfsuite.session.modelPreferencesFile, wfsuite.session.modelPreferences)
        end

        wfsuite.bus.notify("dashboard.reload_themes", {force = true})
...
local function wakeup()
    ...
    if currState ~= prevConnectedState then
        ...
        if currState then
            generateThemeList()
            local f = wfsuite.app.formFields[fieldIds.model_preflight]
            if f and f.values then f:values(formattedThemesModel) end
            f = wfsuite.app.formFields[fieldIds.model_inflight]
            if f and f.values then f:values(formattedThemesModel) end
            f = wfsuite.app.formFields[fieldIds.model_postflight]
            if f and f.values then f:values(formattedThemesModel) end
            if type(settings_model) == "table" and settings_model.use_same_theme ~= nil then
                modelUseSameOverride = settings_model.use_same_theme == true or settings_model.use_same_theme == "true"
            else
