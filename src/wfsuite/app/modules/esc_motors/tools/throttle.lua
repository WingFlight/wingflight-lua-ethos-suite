...
local function wakeup() 
    ...
    local protocolValue = wfsuite.app.Page.apidata.formdata.fields[FIELDKEY.PROTOCOL].value
    ...
    local unsyncedValue = wfsuite.app.Page.apidata.formdata.fields[FIELDKEY.UNSYNCED].value
    ...
    if unsyncedValue == nil then
        wfsuite.app.Page.apidata.formdata.fields[FIELDKEY.UNSYNCED].value = 0
    end
    ...
    elseif(protocolValue == 8) then  -- PROSHOT
        ...
        formFields[FIELDKEY.MAXTHROTTLE]:enable(false)
        formFields[FIELDKEY.UNSYNCED]:enable(false)        
    elseif(protocolValue == 9 and wfsuite.utils.apiVersionCompare(">=", {12, 0, 7})) then  -- CASTLE
        formFields[FIELDKEY.PWM_RATE]:enable(true)
        formFields[FIELDKEY.MINCOMMAND]:enable(true)
        formFields[FIELDKEY.MINTHROTTLE]:enable(true)
        formFields[FIELDKEY.MAXTHROTTLE]:enable(true)
