...
local function startLoad()
    ...
    wfsuite.app.ui.progressDisplay("@i18n(app.modules.configuration.name)@", "@i18n(app.modules.configuration.progress_loading)@", 0.08)

    startRead(wfsuite.tasks.msp.api.loadPage("NAME"), {
    ...
    startRead(wfsuite.tasks.msp.api.loadPage("ADVANCED_CONFIG"), {
    ...
    startRead(wfsuite.tasks.msp.api.loadPage("FEATURE_CONFIG"), {
    ...
    startRead(wfsuite.tasks.msp.api.loadPage("STATUS"), {
        onComplete = function(api)
            local parsed = api.data() and api.data().parsed or nil
...
local function performSave()
    ...
    state.saving = true
    state.saveError = nil
    wfsuite.app.ui.progressDisplaySave("@i18n(app.modules.configuration.progress_saving)@")

    local nameApi = wfsuite.tasks.msp.api.loadPage("NAME")
    if not nameApi then
        saveFailed("@i18n(app.modules.configuration.error_name_api_unavailable)@")
        return
    end

    local advApi = wfsuite.tasks.msp.api.loadPage("ADVANCED_CONFIG")
    if not advApi then
        saveFailed("@i18n(app.modules.configuration.error_advanced_api_unavailable)@")
        return
    end

    local featureApi = wfsuite.tasks.msp.api.loadPage("FEATURE_CONFIG")
