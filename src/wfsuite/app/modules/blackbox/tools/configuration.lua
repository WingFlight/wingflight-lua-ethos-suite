...
local function updateVisibility()
    ...
    if state.form.initialErase and state.form.initialErase.enable then
        state.form.initialErase:enable(edit and device == 1 and wfsuite.utils.apiVersionCompare(">=", {12, 0, 8}))
    end

    if state.form.rollingErase and state.form.rollingErase.enable then
        state.form.rollingErase:enable(edit and device == 1 and wfsuite.utils.apiVersionCompare(">=", {12, 0, 8}))
    end

    if state.form.gracePeriod and state.form.gracePeriod.enable then
        state.form.gracePeriod:enable(edit and device ~= 0 and (mode == 1 or mode == 2) and wfsuite.utils.apiVersionCompare(">=", {12, 0, 8}))
    end
...
local function onReadDone()
    ...
    if state.pendingReads <= 0 then
        if wfsuite.session then
            if not wfsuite.session.blackbox then wfsuite.session.blackbox = {} end
            wfsuite.session.blackbox.feature = {enabledFeatures = state.featureBitmap or 0}
            wfsuite.session.blackbox.config = {
            ...
            wfsuite.session.blackbox.media = {
                dataflashSupported = state.media.dataflashSupported,
                sdcardSupported = state.media.sdcardSupported
            }
            wfsuite.session.blackbox.ready = tonumber(state.cfg.blackbox_supported or 0) == 1
        end
        state.loading = false
        state.loaded = true
        renderForm()
...
local function loadFromSessionSnapshot()
    local snapshot = wfsuite.session and wfsuite.session.blackbox or nil
...
local function performSave()
    ...
    API.setCompleteHandler(function()
        ...
        EAPI.setCompleteHandler(function()
            state.saving = false
            state.dirty = false
            if wfsuite.session and wfsuite.session.blackbox then
                wfsuite.session.blackbox.config = {
                    blackbox_supported = state.cfg.blackbox_supported,
                    device = state.cfg.device,
                    mode = state.cfg.mode,
