...
local findTimeoutDefault = math.floor(wfsuite.tasks.msp.protocol.pageReqTimeout * 0.5)
...
local function scheduleSelectorGuardRetry(reason)
    ...
    if reason and wfsuite.utils and wfsuite.utils.log then
        wfsuite.utils.log("ESC 4WIF selector guard retry: " .. tostring(reason), "info")
    end
end
...
local function scheduleEscReadRecovery()
    ...
    if wfsuite.utils and wfsuite.utils.log then
        wfsuite.utils.log("ESC 4WIF read retry re-arm #" .. tostring(escReadRecoverCount), "info")
    end
end
...
local function pressToolButton(childIdx)
    ...
    if wfsuite.session then
        wfsuite.session.esc4WaySkipEntrySwitchOnce = true
    end
    wfsuite.app.ui.openPage(childOpts)
end
...
local function ensureTailMode(callback)
    local helpers = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.helpers
    ...
    if callback then callback(wfsuite.session and wfsuite.session.tailMode or nil) end
end

local function isPostConnectComplete()
    return wfsuite.session and wfsuite.session.postConnectComplete == true
end
...
local function updateEsc2AvailabilityFromCount(count)
    ...
    if wfsuite.session then wfsuite.session.esc4WayMotorCount = n end
    esc2Available = n >= 2
    return true
end

local function resolveEsc2AvailabilityFromSession()
    if wfsuite.session and updateEsc2AvailabilityFromCount(wfsuite.session.esc4WayMotorCount) then
        return true
    end
    return false
end
...
local function requestEsc2AvailabilityCheck()
    ...
    if wfsuite.utils and wfsuite.utils.uuid then
        API.setUUID(wfsuite.utils.uuid())
    else
...
local function setESC4WayMode(id)
    ...
    if not escSwitchApi then
        escSwitchApi = wfsuite.tasks.msp.api.loadPage("4WIF_ESC_FWD_PROG")
    end
    local API = escSwitchApi
    if not API then return false, "api_missing" end
    if wfsuite.utils and wfsuite.utils.log then
        wfsuite.utils.log("ESC 4WIF set target: " .. tostring(target), "info")
    end
    wfsuite.session.esc4WayTarget = target
    wfsuite.session.esc4WaySetComplete = false
    API.setValue("target", target)
    API.setCompleteHandler(function(self, buf)
        if seq == lastWriteSeq then
            last4WayWriteOk = true
            wfsuite.session.esc4WaySetComplete = true
        end
    end)
    API.setErrorHandler(function(self, err)
        ...
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ESC 4WIF set target: " .. tostring(target) .. " failed", "info")
        end
    end)
    if wfsuite.utils and wfsuite.utils.uuid then
        API.setUUID(wfsuite.utils.uuid())
...
local function beginEscSwitch(target, opts)
    ...
    if wfsuite.app and wfsuite.app.ui and wfsuite.app.ui.progressDisplay then
        wfsuite.app.ui.progressDisplay("@i18n(app.modules.esc_tools.name)@", "@i18n(app.msg_loading)@", wfsuite.app.loaderSpeed.VSLOW)
        switchLoadingActive = true
        wfsuite.app.triggers.closeProgressLoader = false
    end
    last4WayWriteOk = nil
    last4WayWriteTarget = nil
    escSwitchReadFlushPending = false
    escReadReadyAt = nil
    if wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.mspQueue then
        wfsuite.tasks.msp.mspQueue:clear()
    end
    switchState = {
        target = target,
        attempts = 0,
    ...
    wfsuite.session.esc4WaySelected = false
    wfsuite.session.esc4WaySet = false
    wfsuite.session.esc4WaySetComplete = false
...
local function pressSelectorButton(childIdx)
    ...
    inSelector = false
    wfsuite.preferences.menulastselected["esc4way"] = childIdx
    local loaderSpeed = ((wfsuite.app.loaderSpeed and wfsuite.app.loaderSpeed.VSLOW) or 0.5) * 0.15
    wfsuite.app.ui.progressDisplay(nil, nil, loaderSpeed)
...
local function processEscSwitch()
    ...
    if writeResult == true then
        ...
        wfsuite.session.esc4WayTarget = switchState.target
        wfsuite.session.esc4WaySelected = true
        wfsuite.session.esc4WaySet = true
    ...
    if switchState.attempts >= switchState.maxAttempts then
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ESC 4WIF set target failed after retries", "info")
        end
        switchState = nil
...
local function getSelectedEsc4WayTarget()
    local target = tonumber(wfsuite.session and wfsuite.session.esc4WayTarget)
...
local function loadEscConfig(folder)
    ...
    if not chunk then
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ESC config load failed: " .. tostring(initPath) .. " (" .. tostring(err) .. ")", "info")
        end
        return false
    end
    ...
    if not ok or type(moduleOrErr) ~= "table" then
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ESC config init failed: " .. tostring(initPath) .. " (" .. tostring(moduleOrErr) .. ")", "info")
        end
        return false
    end
    ESC = moduleOrErr
...
openSelector = function()
    ...
    local prevTarget = wfsuite.session and wfsuite.session.esc4WayTarget or nil
    ...
    wfsuite.session.esc4WaySelected = false
    wfsuite.session.esc4WaySet = nil
    wfsuite.session.esc4WaySetComplete = nil
    ...
    if wfsuite.app.gfx_buttons["esc4way"] == nil then wfsuite.app.gfx_buttons["esc4way"] = {} end
    if wfsuite.preferences.menulastselected["esc4way"] == nil then wfsuite.preferences.menulastselected["esc4way"] = 1 end
    if esc2Available ~= true and wfsuite.preferences.menulastselected["esc4way"] == 2 then
        wfsuite.preferences.menulastselected["esc4way"] = 1
    end
    ...
    for childIdx, item in ipairs(items) do
        ...
        if wfsuite.preferences.general.iconsize ~= 0 then
            if wfsuite.app.gfx_buttons["esc4way"][childIdx] == nil then
                wfsuite.app.gfx_buttons["esc4way"][childIdx] = loadMask("app/modules/esc_tools/tools/escmfg/" .. folder .. "/gfx/" .. item.image)
            end
        else
            wfsuite.app.gfx_buttons["esc4way"][childIdx] = nil
        end
        ...
        wfsuite.app.formFields[childIdx] = form.addButton(nil, {x = bx, y = y, w = buttonW, h = buttonH}, {
            text = item.title,
            icon = wfsuite.app.gfx_buttons["esc4way"][childIdx],
            options = FONT_S,
            paint = noop,
            press = getSelectorButtonHandler(childIdx)
        })

        if wfsuite.preferences.menulastselected["esc4way"] == childIdx then wfsuite.app.formFields[childIdx]:focus() end
...
local function openPage(opts)
    ...
    if not folder then
        if wfsuite.utils and wfsuite.utils.log then
            wfsuite.utils.log("ESC folder resolution failed for title: " .. tostring(title), "info")
        end
        wfsuite.app.triggers.closeProgressLoader = true
        return
    end
    ...
    if wfsuite.session and wfsuite.session.esc4WaySkipEntrySwitchOnce == true then
        skipEntrySwitchOnce = true
    end
    if wfsuite.session then
        wfsuite.session.esc4WaySkipEntrySwitchOnce = nil
    end

    if not wfsuite.session.esc4WaySelected then
        openSelector()
        return
    end
...
local function onNavMenu()
    ...
    if ESC then
        if not inSelector then
            wfsuite.session.esc4WaySelected = nil
            wfsuite.session.esc4WaySet = nil
            wfsuite.session.esc4WaySetComplete = nil
            wfsuite.session.esc4WayTarget = nil
            openSelector()
            return true
        end
        wfsuite.session.esc4WaySelected = nil
...
local function closePage()
    local keepEscSessionHot = wfsuite.session and wfsuite.session.esc4WaySkipEntrySwitchOnce == true

    if switchLoadingActive then
        switchLoadingActive = false
        if wfsuite.app and wfsuite.app.triggers then
            wfsuite.app.triggers.closeProgressLoader = true
        end
    end

    if wfsuite.app and wfsuite.app.gfx_buttons then
        wfsuite.app.gfx_buttons["esctool"] = nil
        wfsuite.app.gfx_buttons["esc4way"] = nil
    end
...
local function onReloadMenu()
    ...
    if wfsuite.session then
        wfsuite.session.esc4WaySkipEntrySwitchOnce = nil
        wfsuite.session.esc4WayTarget = nil
        wfsuite.session.esc4WayMotorCount = nil
        wfsuite.session.esc4WaySelected = nil
        wfsuite.session.esc4WaySet = nil
        wfsuite.session.esc4WaySetComplete = nil
    end
...
local function wakeup()
    ...
    if foundESC == false then
        ...
        if wfsuite.session.esc4WaySet == true and wfsuite.session.esc4WaySetComplete == true then
    ...
    if foundESC == true and foundESCupdateTag == false then
        ...
        if ESC and type(ESC.isCompatibleEsc) == "function" then
            compatible = (ESC.isCompatibleEsc(wfsuite.session and wfsuite.session.escBuffer or nil, escDetailsApi) == true)
        end
        ...
        if escDetails.model ~= nil and escDetails.model ~= nil and escDetails.firmware ~= nil then
            local prefix = ""
            local target = wfsuite.session and wfsuite.session.esc4WayTarget or 0
    ...
    if wfsuite.app and wfsuite.app.dialogs and wfsuite.app.dialogs.progressDisplay == true then
        progressTimedOut = (wfsuite.app.dialogs.progressCounter or 0) >= 101
    end

    if foundESCupdateTag == false and ((findTimeoutClock <= os.clock() - findTimeout) or progressTimedOut) then
        local dialogs = wfsuite.app and wfsuite.app.dialogs
        if dialogs then
