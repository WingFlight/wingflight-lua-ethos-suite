...
if wfsuite.session.escDetails and wfsuite.session.escDetails.model then

    local TEST_150A = false

    if string.find(wfsuite.session.escDetails.model, "FLYROTOR 150A") or TEST_150A == true then
...
local function postLoad() wfsuite.app.triggers.closeProgressLoader = true end
