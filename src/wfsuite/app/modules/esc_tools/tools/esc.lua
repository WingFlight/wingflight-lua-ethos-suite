...
local function showEntryWarningDialog(title)
    ...
    if wfsuite.utils and wfsuite.utils.playFileCommon then
...
local function shouldQueueEntryWarning(opts)
    if not prefBool(wfsuite.preferences.general.show_esc_tools_warning, true) then
        return false
    end
    if type(opts) ~= "table" then
        return false
    end
    if wfsuite.app and wfsuite.app._openedFromShortcuts == true then
        return false
    end
...
local function canDetectEscProtocol()
    ...
    return wfsuite.session.apiVersion
        and wfsuite.utils.apiVersionCompare(">=", ESC_SENSOR_CONFIG_MIN_API_VERSION)
        and type(api) == "table"
        and type(api.load) == "function"
end

local function computeEntryDisabled(entry)
    if entry.apiversion and wfsuite.session.apiVersion and not wfsuite.utils.apiVersionCompare(">=", entry.apiversion) then
        return true
    end
...
local function pressMainButton(childIdx)
    ...
    wfsuite.preferences.menulastselected["escmain"] = childIdx
    wfsuite.app.ui.progressDisplay(nil, nil, 0.5)
...
local function getStoredSelectedIdx()
    local selectedIdx = tonumber(wfsuite.preferences.menulastselected["escmain"]) or 1
    if selectedIdx < 1 then selectedIdx = 1 end
    if selectedIdx > #pages then selectedIdx = #pages end
    if selectedIdx < 1 then selectedIdx = 1 end
    wfsuite.preferences.menulastselected["escmain"] = selectedIdx
    return selectedIdx
end
...
local function applyButtonStates()
    ...
    for childIdx, entry in ipairs(MFG_INDEX) do
        ...
        local button = wfsuite.app.formFields[childIdx]
    ...
    if focusIdx == nil then
        local menuButton = wfsuite.app.formNavigationFields and wfsuite.app.formNavigationFields.menu
...
local function isEscPageActive()
    return wfsuite.app and wfsuite.app.Page and wfsuite.app.Page.openPage == openPage
end
...
local function requestEscProtocol()
    ...
    local API = wfsuite.tasks.msp.api.loadPage(ESC_SENSOR_CONFIG_API)
    if not API then
    ...
    API.setCompleteHandler(function()
        ...
        local value = wfsuite.utils.getEffectiveEscSensorProtocol(API.readValue and API.readValue("protocol") or nil)
        if value ~= nil then
...
openPage = function(opts)
    ...
    local mspProtocol = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.protocol
    ...
    if wfsuite.app.gfx_buttons["escmain"] == nil then wfsuite.app.gfx_buttons["escmain"] = {} end
    if wfsuite.preferences.menulastselected["escmain"] == nil then wfsuite.preferences.menulastselected["escmain"] = 1 end
    ...
    for childIdx, pvalue in ipairs(pages) do
        ...
        if wfsuite.preferences.general.iconsize ~= 0 then
            if wfsuite.app.gfx_buttons["escmain"][childIdx] == nil then
                wfsuite.app.gfx_buttons["escmain"][childIdx] = loadMask("app/modules/esc_tools/tools/escmfg/" .. pvalue.folder .. "/" .. pvalue.image)
            end
        else
            wfsuite.app.gfx_buttons["escmain"][childIdx] = nil
        end
        ...
        wfsuite.app.formFields[childIdx] = form.addButton(nil, {x = bx, y = y, w = buttonW, h = buttonH}, {
            ...
            icon = wfsuite.app.gfx_buttons["escmain"][childIdx],
            options = FONT_S,
            paint = noop,
...
local function wakeup()
    ...
    local title = entryWarningTitle or wfsuite.app.lastTitle or "@i18n(app.modules.esc_tools.name)@"
    clearEntryWarningState()
...
closePage = function()
    ...
    if wfsuite.app and wfsuite.app.gfx_buttons then
        wfsuite.app.gfx_buttons["escmain"] = nil
    end
    pages = {}
    clearButtonCache()
    clearEscMaskCache()
