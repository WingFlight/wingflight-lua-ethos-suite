...
local navHandlers = pageRuntime.createMenuHandlers({
    ...
    showProgress = true,
    progressSpeed = wfsuite.app.loaderSpeed.FAST
})
...
local function readData()
    ...
    if not boardAPI or not sensorAPI then
        wfsuite.utils.log("Alignment read failed: API unavailable", "error")
        return
    end
    ...
    boardAPI.setCompleteHandler(function()
        ...
        sensorAPI.setErrorHandler(function()
            wfsuite.utils.log("Alignment read failed: SENSOR_ALIGNMENT", "error")
        end)
    ...
    boardAPI.setErrorHandler(function()
        wfsuite.utils.log("Alignment read failed: BOARD_ALIGNMENT_CONFIG", "error")
    end)
...
local function writeData()
    ...
    if not boardAPI or not sensorAPI or not eepromAPI then
        ...
        wfsuite.utils.log("Alignment save failed: API unavailable", "error")
        return
    end
    ...
    boardAPI.setCompleteHandler(function()
        sensorAPI.setCompleteHandler(function()
            ...
            eepromAPI.setErrorHandler(function()
                state.saving = false
                app.triggers.closeProgressLoader = true
                wfsuite.utils.log("Alignment save failed: EEPROM_WRITE", "error")
            end)
            eepromAPI.write()
        end)
        sensorAPI.setErrorHandler(function()
            state.saving = false
            app.triggers.closeProgressLoader = true
            wfsuite.utils.log("Alignment save failed: SENSOR_ALIGNMENT", "error")
        end)
        sensorAPI.write()
    end)
    boardAPI.setErrorHandler(function()
        state.saving = false
        app.triggers.closeProgressLoader = true
        wfsuite.utils.log("Alignment save failed: BOARD_ALIGNMENT_CONFIG", "error")
    end)
    boardAPI.write()
end
