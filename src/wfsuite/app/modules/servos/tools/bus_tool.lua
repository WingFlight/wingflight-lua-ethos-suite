...
local servoIndex = wfsuite.currentServoIndex - 1 -- UI 0-based index (0..N-1)
...
local function uiIndexToConfigWriteIndex(ui0)
    ...
    return (ui0 or 0) + (wfsuite.session.servoCount - BUS_OUTPUT_COUNT)
end
...
local function saveServoCenter(self)
    ...
    local writeIndex = currentServoWriteIndex()
    wfsuite.utils.log(string.format("BUS save center: ui=%d read=%d write=%d mid=%d", servoIndex, currentServoReadIndex(), writeIndex, servoCenter), "debug")
        ...
        if wfsuite.session.servoOverride == true then
            ...
            if ((now - lastServoChangeTime) >= settleTime) and wfsuite.tasks.msp.mspQueue:isProcessed() then
                if currentServoCenter ~= lastSetServoCenter then
                    ...
                    elseif reason then
                        wfsuite.utils.log("BUS servo trim enqueue rejected: " .. tostring(reason), "debug")
                    end    
                end
            end
...
local function openPage(opts)
    ...
    wfsuite.app.ui.fieldHeader("@i18n(app.modules.servos.bus)@" .. " / " .. wfsuite.app.utils.titleCase(configs[servoIndex]['name']))
    wfsuite.app.ui.setPageDirty(false)
