...
local function writeEeprom()
    ...
    if not ok then
        wfsuite.utils.log("Servo BUS EEPROM enqueue rejected: " .. tostring(reason), "info")
    end
    return ok, reason
end
...
local function openPage(opts)
    ...
    wfsuite.app.ui.fieldHeader(title or "@i18n(app.modules.servos.name)@ / @i18n(app.modules.servos.bus)@")
    ...
    if wfsuite.app.gfx_buttons["bus"] == nil then wfsuite.app.gfx_buttons["bus"] = {} end
    if wfsuite.preferences.menulastselected["bus"] == nil then wfsuite.preferences.menulastselected["bus"] = 1 end
    ...
    for pidx, pvalue in ipairs(servoTable) do
        ...
        if pvalue.disabled ~= true then
            ...
            if wfsuite.preferences.general.iconsize ~= 0 then
                if wfsuite.app.gfx_buttons["bus"][pidx] == nil then wfsuite.app.gfx_buttons["bus"][pidx] = loadMask("app/modules/servos/gfx/" .. pvalue.image) end
            else
                wfsuite.app.gfx_buttons["bus"][pidx] = nil
            end

            wfsuite.app.formFields[pidx] = form.addButton(nil, {x = bx, y = y, w = buttonW, h = buttonH}, {
                text = pvalue.title,
                icon = wfsuite.app.gfx_buttons["bus"][pidx],
                options = FONT_S,
                paint = function() end,
                press = function()
                    wfsuite.preferences.menulastselected["bus"] = pidx
            ...
            if wfsuite.preferences.menulastselected["bus"] == pidx then wfsuite.app.formFields[pidx]:focus() end
...
local function servoCenterFocusAllOn(self)
    ...
    if wfsuite.utils.apiVersionCompare(">=", {12, 0, 9}) then
        ...
        for i = 0, #servoTable do
            queueServoOverride(i, 0, string.format("servo.bus.override.%d.on", i))
        end
    end    
...
local function servoCenterFocusAllOff(self)
    ...
    if wfsuite.utils.apiVersionCompare(">=", {12, 0, 9}) then
        ...
        for i = 0, #servoTable do
            queueServoOverride(i, 2001, string.format("servo.bus.override.%d.off", i))
        end
    end    
    wfsuite.app.triggers.isReady = true
    wfsuite.app.triggers.closeProgressLoader = true
