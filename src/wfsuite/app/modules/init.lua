...
local function developerToolsEnabled()
    local pref = wfsuite.preferences and wfsuite.preferences.general
    return pref and isTruthy(pref.developer_tools) or false
end

local function shortcutsEnabled()
    local prefs = wfsuite.preferences and wfsuite.preferences.shortcuts
...
local function resolveLoaderSpeed(value)
    ...
    if type(value) == "string" then
        local speeds = wfsuite.app and wfsuite.app.loaderSpeed
...
local function includeByFeature(spec)
    ...
    if not wfsuite.features then return true end
    return wfsuite.features.isEnabled(feature, wfsuite.preferences)
end
...
local function addLeafPage(sectionIndex, spec)
    ...
    if not (page.folder and page.script) then
        wfsuite.utils.log("Manifest page missing folder/module or script: " .. tostring(page.title), "info")
        return
    end
...
local function resolveShortcutSections()
    ...
    local prefs = wfsuite.preferences and wfsuite.preferences.shortcuts or {}
