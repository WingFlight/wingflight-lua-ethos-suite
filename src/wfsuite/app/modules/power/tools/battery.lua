...
local function setFieldColorByApiKey(apikey, color)
    local idx = fieldIndexByApiKey[apikey]
    if not idx then return end
    local field = wfsuite.app.formFields and wfsuite.app.formFields[idx]
...
local function saveProfileCapacity(profileIndex, capacity)
    ...
    if wfsuite.session.batteryConfig and wfsuite.session.batteryConfig.profiles then
        wfsuite.session.batteryConfig.profiles[profileIndex] = finalVal
    end
end
...
local function resetSmartfuel()
    local sensors = wfsuite.tasks and wfsuite.tasks.sensors
    if sensors and type(sensors.resetSmart) == "function" then
        sensors.resetSmart()
    end

    local eventTelemetry = wfsuite.tasks and wfsuite.tasks.events and wfsuite.tasks.events.telemetry
...
local function updateDynamicUi(self, editingType, activeType)
    local headerBase = wfsuite.app.lastTitle or "@i18n(app.modules.power.battery)@"
    local headerProfile = activeType
    if headerProfile ~= nil then
        pcall(function()
            wfsuite.app.ui.setHeaderTitle(headerBase .. " #" .. tostring(headerProfile + 1))
        end)
    else
        pcall(function()
            wfsuite.app.ui.setHeaderTitle(headerBase)
        end)
    end
...
local function postLoad(self)
    ...
    local activeType = clampProfileIndex(wfsuite.session.activeBatteryType)
...
local function wakeup(self)
    ...
    if activeType ~= nil and activeType ~= lastActiveType then
        ...
        if idx and wfsuite.app.formFields and wfsuite.app.formFields[idx] and wfsuite.app.formFields[idx].value then
            wfsuite.app.formFields[idx]:value(activeType)
    ...
    if editingType ~= nil and editingType ~= lastEditingType then
        ...
        if capacity ~= nil then
            ...
            if idx and wfsuite.app.formFields and wfsuite.app.formFields[idx] and wfsuite.app.formFields[idx].value then
                wfsuite.app.formFields[idx]:value(capacity)
                needsInvalidate = true
            end
        end
        lastEditingType = editingType
