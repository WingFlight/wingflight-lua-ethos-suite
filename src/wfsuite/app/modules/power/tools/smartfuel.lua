...
local function getLocalSource()
    local bat = wfsuite.session and wfsuite.session.modelPreferences and wfsuite.session.modelPreferences.battery
...
local function postLoad(self)
    if useFirmwareSmartFuel then
        ...
        wfsuite.session.batteryConfig = wfsuite.session.batteryConfig or {}
        wfsuite.session.batteryConfig.smartfuelRemoteSource = tonumber(sourceField.value) or 0
    end
    lastTuningActive = nil
...
local function writeLocalSmartfuelSettings()
    ...
    local iniFile = "SCRIPTS:/" .. wfsuite.config.preferences .. "/models/" .. mcuId .. ".ini"
    local ini = wfsuite.ini
    ...
    if not ok then
        wfsuite.utils.log("Failed to save local SmartFuel settings: " .. tostring(err or iniFile), "info")
        return
    end
...
local function wakeup(self)
    ...
    for i = TUNING_FIELD_START, #apidata.formdata.fields do
        local fieldHandle = wfsuite.app.formFields[i]
