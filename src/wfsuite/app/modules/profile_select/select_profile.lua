...
local function configureProfileChoice(fieldIndex, count)
    ...
    local formField = wfsuite.app and wfsuite.app.formFields and wfsuite.app.formFields[fieldIndex]
    if formField and formField.values then
        formField:values(wfsuite.app.utils.convertPageValueTable(choices, field.tableIdxInc))
    end
end
...
local function postLoad(self)
    local status = wfsuite.tasks.msp.api.apidata.values and wfsuite.tasks.msp.api.apidata.values.STATUS
    ...
    if wfsuite.app and wfsuite.app.utils then
        wfsuite.app.utils.getCurrentProfile()
        wfsuite.app.utils.getCurrentRateProfile()
    end
...
local function setPidProfile(profileIndex)
    ...
    local API = wfsuite.tasks.msp.api.loadPage("SELECT_PROFILE")
...
local function wakeup()
    ...
    if triggerMSPWrite == true then
        ...
        local profileIndex = wfsuite.app.Page.apidata.formdata.fields[1].value
        local rateIndex = wfsuite.app.Page.apidata.formdata.fields[2].value
        local okRate, reasonRate = setRateProfile(rateIndex)
        if not okRate then
            wfsuite.utils.log("Rate profile enqueue rejected: " .. tostring(reasonRate), "info")
        ...
        if not okPid then
            wfsuite.utils.log("PID profile enqueue rejected: " .. tostring(reasonPid), "info")
            wfsuite.app.triggers.closeSaveFake = true
            wfsuite.app.triggers.isSaving = false
        end
    end
