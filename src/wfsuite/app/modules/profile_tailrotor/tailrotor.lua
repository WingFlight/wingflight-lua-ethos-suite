...
local function setTtaEnabled(enabled)
    ...
    local formFields = wfsuite.app and wfsuite.app.formFields
...
local function postLoad(self)
    ...
    activateWakeup = true
    setTtaEnabled((tonumber(wfsuite.session and wfsuite.session.governorMode) or 0) >= 1)
end

local function wakeup()
    local helpers = wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.helpers
    if wfsuite.session.governorMode == nil and helpers and helpers.governorMode and not governorModeRequested then
