...
local function ensureApis()
    if not calibrationAPI then
        calibrationAPI = wfsuite.tasks.msp.api.loadPage("ACC_CALIBRATION")
        calibrationAPI.setUUID("accelerometer-calibration")
    end
    if not eepromAPI then
        eepromAPI = wfsuite.tasks.msp.api.loadPage("EEPROM_WRITE")
...
local function applySettings()
    ...
    if ok then
        ...
        wfsuite.utils.log("Accelerometer EEPROM enqueue rejected: " .. tostring(reason), "info")
    end
end
...
local function wakeup()
    ...
    if calibrate == true and calibrateQueued == false then
        ...
        if ok then
            ...
            wfsuite.utils.log("Accelerometer calibration enqueue rejected: " .. tostring(reason), "info")
            calibrate = false
        end
    ...
    if calibrateQueued == true and calibrationAPI and calibrationAPI.writeComplete() then
        wfsuite.utils.log("Accelerometer calibrated.", "info")
        calibrate = false
        calibrateQueued = false
        applySettings()
    end
