...
local PORT_FUNCTIONS = {
    ...
    {id = 262144, excl = 262144, name = "@i18n(app.modules.ports.function_sbus_out)@", type = PORT_TYPE_AUTO, minApi = {22, 0, 0}},
    {id = 524288, excl = 524288, name = "@i18n(app.modules.ports.function_fbus_out)@", type = PORT_TYPE_AUTO, minApi = {22, 0, 0}},
    {id = 1048576, excl = 1048576, name = "@i18n(app.modules.ports.function_sport_input)@", type = PORT_TYPE_AUTO, minApi = {22, 0, 0}},
...
local function readRxConfig(done)
    local rxApi = wfsuite.tasks.msp.api.loadPage("RX_CONFIG")
    if not rxApi then
...
local function startLoad()
    ...
    wfsuite.app.ui.progressDisplay("@i18n(app.modules.ports.name)@", "@i18n(app.modules.ports.progress_loading)@", 0.08)

    local serialApi = wfsuite.tasks.msp.api.loadPage("SERIAL_CONFIG")
...
local function queueSetSerialPort(port, done, failed)
    local API = wfsuite.tasks.msp.api.loadPage("SERIAL_CONFIG")
    if not API then
...
local function savePorts()
    ...
    wfsuite.app.ui.progressDisplay("@i18n(app.modules.ports.name)@", "@i18n(app.modules.ports.progress_saving)@", 0.08)
