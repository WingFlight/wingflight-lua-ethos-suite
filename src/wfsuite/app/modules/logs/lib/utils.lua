...
function utils.resolveModelName(foldername)
    ...
    local iniName = "LOGS:wfsuite/telemetry/" .. foldername .. "/logs.ini"
    local iniData = wfsuite.ini.load_ini_file(iniName) or {}
...
function utils.getLogPath(dirname)
    ...
    os.mkdir("LOGS:/wfsuite/telemetry")

    local targetDir = dirname or wfsuite.app.activeLogDir
    if targetDir then return string.format("LOGS:/wfsuite/telemetry/%s/", targetDir) end
    return "LOGS:/wfsuite/telemetry/"
end
...
function utils.getLogDir(dirname)
    ...
    if not dirname then
        local defaultDir = "LOGS:/wfsuite/telemetry/" .. wfsuite.session.mcu_id .. "/"
        os.mkdir(defaultDir)
        return defaultDir
    end

    return "LOGS:/wfsuite/telemetry/" .. dirname .. "/"
end
