...
local function wakeup(self)
    ...
    if not wfsuite.app.Page or not wfsuite.app.Page.apidata or not wfsuite.app.Page.apidata.formdata then return end

    local pageFields = wfsuite.app.Page.apidata.formdata.fields

    for i = 1, #channelLabels do
        local modeFieldIndex = modeFieldIndexByChannel[i]
        local valueFieldIndex = valueFieldIndexByChannel[i]
        local modeField = pageFields and pageFields[modeFieldIndex]
        local valueWidget = wfsuite.app.formFields[valueFieldIndex]
