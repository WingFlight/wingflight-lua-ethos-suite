...
local total_bytes = normalizeByteCount(wfsuite.preferences.developer.mspexpbytes)
...
local function buildFieldMap()
    fieldMap = {}
    for _, field in ipairs(wfsuite.app.Page.apidata.formdata.fields or {}) do
...
local function update_int8()
    ...
    for _, field in ipairs(wfsuite.app.Page.apidata.formdata.fields or {}) do if field.isINT8 then for _, match in ipairs(fieldMap[field.label] or {}) do if match.isUINT8 then field.value = uint8_to_int8(safeFieldValue(match)) end end end end
end
...
local function update_uint8()
    ...
    for _, field in ipairs(wfsuite.app.Page.apidata.formdata.fields or {}) do if field.isUINT8 then for _, match in ipairs(fieldMap[field.label] or {}) do if match.isINT8 then field.value = int8_to_uint8(safeFieldValue(match)) end end end end
end
...
local function postLoad()
    local receivedBytes = wfsuite.tasks.msp.api.apidata.receivedBytesCount
    ...
    if total_bytes ~= receivedByteCount then
        wfsuite.preferences.developer.mspexpbytes = receivedByteCount
...
local function wakeup()
    if not enableWakeup or not wfsuite.app.Page or not wfsuite.app.Page.apidata.formdata.fields then return end
    periodicSync()
end
