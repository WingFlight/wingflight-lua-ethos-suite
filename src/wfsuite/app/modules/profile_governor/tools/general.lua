...
apidata = {
    ...
    formdata = {
        ...
        fields = {
            {t = "@i18n(app.modules.profile_governor.full_headspeed)@", mspapi = 1, apikey = "governor_headspeed", enablefunction = function() return (wfsuite.session.governorMode >= 2) end}, {t = "@i18n(app.modules.profile_governor.min_throttle)@", mspapi = 1, apikey = "governor_min_throttle", enablefunction = function() return (wfsuite.session.governorMode >= 2) end},
            {t = "@i18n(app.modules.profile_governor.max_throttle)@", mspapi = 1, apikey = "governor_max_throttle", enablefunction = function() return (wfsuite.session.governorMode >= 1) end}, {t = "@i18n(app.modules.profile_governor.fallback_drop)@", mspapi = 1, apikey = "governor_fallback_drop", enablefunction = function() return (wfsuite.session.governorMode >= 1) end},
            {t = "@i18n(app.modules.profile_governor.gain)@", mspapi = 1, apikey = "governor_gain", enablefunction = function() return (wfsuite.session.governorMode >= 2) end}, {t = "@i18n(app.modules.profile_governor.p)@", inline = 4, label = 1, mspapi = 1, apikey = "governor_p_gain", enablefunction = function() return (wfsuite.session.governorMode >= 2) end},
...
local function wakeup()
    ...
    if activateWakeup == true and wfsuite.tasks.msp.mspQueue:isProcessed() then
        ...
        if wfsuite.session.governorMode == 0 then
        ...
        if decodedFlags["tx_precomp_curve"] then
            wfsuite.app.formFields[FIELD_F_GAIN]:enable(false)
            wfsuite.app.formFields[FIELD_YAW_WEIGHT]:enable(false)
            wfsuite.app.formFields[FIELD_CYCLIC_WEIGHT]:enable(false)
            wfsuite.app.formFields[FIELD_COLLECTIVE_WEIGHT]:enable(false)

        else
            wfsuite.app.formFields[FIELD_F_GAIN]:enable(true)
            wfsuite.app.formFields[FIELD_YAW_WEIGHT]:enable(true)
            wfsuite.app.formFields[FIELD_CYCLIC_WEIGHT]:enable(true)
            wfsuite.app.formFields[FIELD_COLLECTIVE_WEIGHT]:enable(true)
        end
