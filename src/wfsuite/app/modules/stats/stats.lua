...
local function postSave()
    ...
    local totalflighttime = toNumber(wfsuite.ini.getvalue(prefs, "general", "totalflighttime"), 0)
    local flightcount     = toNumber(wfsuite.ini.getvalue(prefs, "general", "flightcount"), 0)
    ...
    local API = wfsuite.tasks.msp.api.loadPage("FLIGHT_STATS")
        ...
        for i,v in pairs(FBL_STATS) do
            wfsuite.utils.log("Pre-Sync FBL_STATS: " .. i .. "=" .. tostring(v), "info")
            API.setValue(i, v)
        end
        ...
        API.setCompleteHandler(function()
                ...
                wfsuite.utils.log("PostSave: remote flight stats updated", "connect")
                saveToEeprom()
        end)
        API.write()
    ...
    if wfsuite.utils.apiVersionCompare(">=", {12, 0, 9}) then
        --  load updated stats from FC after load
            ...
            API.setCompleteHandler(function(self, buf)
                ...
                for i,v in pairs(FBL_STATS) do
                    wfsuite.utils.log("Loaded FBL_STATS: " .. i .. "=" .. tostring(v), "info")
                end

                wfsuite.utils.log("Loaded flight stats from FC after load", "info")
