...
local function getMixerCompatibilityStatus()
    local PAPI = wfsuite.tasks.msp.api.loadPage("GET_MIXER_INPUT_PITCH")
    ...
    PAPI.setUUID("mixer-menu-pitch")
    PAPI.read()

    local RAPI = wfsuite.tasks.msp.api.loadPage("GET_MIXER_INPUT_ROLL")
    ...
    RAPI.setUUID("mixer-menu-roll")
    RAPI.read()

    local CAPI = wfsuite.tasks.msp.api.loadPage("GET_MIXER_INPUT_COLLECTIVE")
    ...
    CAPI.setUUID("mixer-menu-collective")
    CAPI.read()
end
    ...
    onWakeup = function()
        ...
        if wfsuite.session.tailMode == nil or wfsuite.session.swashMode == nil then
            if wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.helpers then
                wfsuite.tasks.msp.helpers.mixerConfig(function(tailMode, swashMode)
        ...
        if wfsuite.app.formFields then
            ...
            if enable and not focused then
                focused = true
                local idx = tonumber(wfsuite.preferences.menulastselected["mixer"]) or 1
