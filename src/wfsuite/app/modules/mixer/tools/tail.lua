...
local function tailChanged()
    ...
    wfsuite.session.tailMode = FORMDATA[LAYOUTINDEX.TAIL_ROTOR_MODE]

    needsReboot = true
end

local function isTailMotorizedMode()
    return (tonumber(wfsuite.session.tailMode) or -1) >= 1
end
...
local function writeNext(i)
    ...
    if not apikey then
        ...
        -- reboot if required
        if needsReboot then
            local RAPI = wfsuite.tasks.msp.api.loadPage("REBOOT")
