...
local function setMixerOverrideAudio(enabled)
    local audio = wfsuite.app and wfsuite.app.audio
...
local function loadNext(i)
  ...
  local API = wfsuite.tasks.msp.api.loadPage(IDX)
...
local function writeNext(i, commitToEeprom)
    ...
    local API = wfsuite.tasks.msp.api.loadPage(apikey)
...
local function disableOverrideForExit(showDialog)
    ...
    inOverRide = false
    if showDialog then
        wfsuite.app.ui.progressDisplay("@i18n(app.modules.mixer.swash_override)@", "@i18n(app.modules.mixer.swash_override_disabling)@")
    end
    mixerOff()
end
...
local function wakeup(self)
    ...
    if triggerOverRide == true then
        ...
        if inOverRide == false then
            ...
            wfsuite.app.ui.progressDisplay("@i18n(app.modules.mixer.swash_override)@", "@i18n(app.modules.mixer.swash_override_enabling)@")
            mixerOn()
            inOverRide = true
            lastAppliedDigest = formDigest()
            lastAppliedFields = snapshotFormFields()
    ...
    if triggerSave then
        ...
        wfsuite.app.ui.progressDisplay("@i18n(app.msg_saving_settings)@","@i18n(app.msg_saving_to_fbl)@")
    ...
    if inOverRide == true then
        ...
        if currentDigest ~= lastAppliedDigest and not saveBusy and (now - lastChangeTime) >= liveUpdateInterval and wfsuite.tasks.msp.mspQueue:isProcessed() then
