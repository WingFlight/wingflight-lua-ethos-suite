...
local APP_SESSION_PACKAGE_KEYS = {
  "wfsuite.app.pages.esc_forward_4way",
  "wfsuite.app.pages.esc_forward_vendor",
}
...
local MENUS = {
  ...
  -- Matches the original's own `flight_tuning_menu` manifest entry, minus
  -- Governor -- wingflight-firmware has no MSP_GOVERNOR_CONFIG any more
  -- (heli engine-RPM governor; see lib/msp_pid_profile.lua's header for
  -- the wider wingflight-vs-rotorflight wire-format divergence this
...
-- A deliberately generous margin (10s) so a normal, if slightly slow,
-- device boot never trips this alert as a false positive before the
-- background task has had a real chance to publish its first
-- "task.status" -- widened well past its original 0.75s after that
-- exact false positive was caught live.
