...
function app.reloadMainMenu()
    ...
    if app.tasks and app.tasks.setMainMenuBuildApiVersion then
        app.tasks.setMainMenuBuildApiVersion(wfsuite.session and wfsuite.session.apiVersion or nil)
    end
    return app.MainMenu
end
...
wfsuite.tasks.appRunning = false
...
function app.wakeup_protected()
    ...
    if wfsuite.session.isConnected and not wfsuite.session.postConnectComplete then
        return 
    end
    ...
    if wfsuite.session and wfsuite.session.mspBusy then
...
function app.create()
    ...
    if not app.initialized then
        ...
        wfsuite.tasks.escPowerCycleLoader = false
        ...
        wfsuite.session.rebootInProgress = false
...
function app.event(widget, category, value, x, y)
    ...
    if app.uiState == app.uiStatus.pages then
        ...
        if value == KEY_ENTER_LONG then
            ...
            local dirtyPref = wfsuite.preferences and wfsuite.preferences.general and wfsuite.preferences.general.save_dirty_only
...
function app.close()
    ...
    wfsuite.utils.reportMemoryUsage("app.close", "start")

    local userpref_file = "SCRIPTS:/" .. wfsuite.config.preferences .. "/preferences.ini"
    wfsuite.ini.save_ini_file(userpref_file, wfsuite.preferences)

    wfsuite.tasks.appRunning = false
    wfsuite.tasks.uiCallbacks = nil
    ...
    if wfsuite.utils and wfsuite.utils.clearImageCaches then
        local ok, err = pcall(wfsuite.utils.clearImageCaches)
        if not ok then
    ...
    wfsuite.config.useCompiler = true
    ...
    if wfsuite.tasks.msp then wfsuite.tasks.msp.api.resetApidata() end
    if wfsuite.tasks.msp and wfsuite.tasks.msp.mspQueue then wfsuite.tasks.msp.mspQueue:clear() end

    collectgarbage("collect")

    wfsuite.utils.reportMemoryUsage("app.close", "end")
