...
local RAM_CRITICAL_FREE_KB = 150
...
local function desiredFocusIndex(app)
    ...
    return wfsuite.preferences.menulastselected[lm] or wfsuite.preferences.menulastselected["mainmenu"]
end
...
local function profileRateChangeDetection()
    local app = wfsuite.app
    if not (app.Page and (app.Page.refreshOnProfileChange or app.Page.refreshOnRateChange or app.Page.refreshFullOnProfileChange or app.Page.refreshFullOnRateChange) and app.uiState == app.uiStatus.pages and not app.triggers.isSaving and not app.dialogs.saveDisplay and not app.dialogs.progressDisplay and wfsuite.tasks.msp.mspQueue:isProcessed()) then return end
...
local function mainMenuIconEnableDisable()
    ...
    local currentApiVersion = wfsuite.session and wfsuite.session.apiVersion
    ...
    if app.uiState == app.uiStatus.mainMenu then
        ...
        local tasksActive = wfsuite.tasks and wfsuite.tasks.active and wfsuite.tasks.active()
        local apiV = tostring(wfsuite.session.apiVersion)
        local connected = (wfsuite.session and wfsuite.session.isConnected) == true
        local postConnectComplete = (wfsuite.session and wfsuite.session.postConnectComplete) == true
        local supportedApi = wfsuite.session.apiVersion and wfsuite.utils.stringInArray(wfsuite.config.supportedMspApiVersion, apiV)
        local modeTag
    ...
    elseif not app.isOfflinePage and not wfsuite.tasks.escPowerCycleLoader then
        if not wfsuite.session.postConnectComplete or wfsuite.session.apiVersion == nil then
...
local function triggerSaveDialogs()
    ...
    if app.triggers.triggerSave then
        ...
        if wfsuite.preferences.general.save_confirm == true or wfsuite.preferences.general.save_confirm == "true" then
            form.openDialog({
                width = nil,
    ...
    if app.triggers.isSaving then
        if app.pageState >= app.pageStatus.saving and not app.dialogs.saveDisplay then
            ...
            wfsuite.tasks.msp.mspQueue.retryCount = 0
        end
    end
end
...
local function armedSaveWarning()
    ...
    local pref = wfsuite.preferences.general.save_armed_warning
    ...
    if not app.dialogs.progressDisplay then
        app.audio.playSaveArmed = true
        app.dialogs.progressCounter = 0
        local key = wfsuite.utils.getArmedSaveBlockedMessage()
...
local function triggerReloadDialogs()
    ...
    if app.triggers.triggerReload then
        ...
        if wfsuite.preferences.general.reload_confirm == false or wfsuite.preferences.general.reload_confirm == "false" then
            app.triggers.reload = true;
            return
        else 
            form.openDialog({
...
local function telemetryAndPageStateUpdates()
    ...
    elseif app.triggers.isReady and (wfsuite.tasks and wfsuite.tasks.msp and wfsuite.tasks.msp.mspQueue:isProcessed()) and app.Page and app.Page.values then
