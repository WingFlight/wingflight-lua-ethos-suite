...
if package.loaded["wfsuite.app.page_runtime"] then
  return package.loaded["wfsuite.app.page_runtime"]
end
...
function PageRuntime.new(config)
  ...
  -- below) for rebootAfterSave and optional armed-save gating -- nil until the
...
function PageRuntime:updateSaveEnabled()
  ...
  local enabled = self:canSave()
  if self.saveEnabledCache == enabled then return end
  self.saveEnabledCache = enabled
  self.headerHandle.setSaveEnabled(enabled)
end
...
function PageRuntime:showLoadError(focusFn)
  if self.disposed or self.loadErrorDialog then return end

  local controlRef = self.controlRef
  self.loadErrorDialog = form.openDialog({
    ...
    close = function()
      ...
      runtime.loadErrorDialog = nil
      if not runtime.disposed and focusFn then focusFn() end
    end,
    wakeup = function() end,
    paint = function() end,
  })
...
function PageRuntime:confirmReload(focusFn)
  if self.disposed or self.busy or self.activeDialog or self.loadErrorDialog then return end
...
function PageRuntime:dispose()
  ...
  local fieldLayout = package.loaded["wfsuite.app.field_layout"]
...
function PageRuntime:buildChrome()
  ...
  if opts.setWakeupHandler then
    opts.setWakeupHandler(function()
      ...
      if runtime.pendingReload and runtime.loaded and not runtime.busy
          and not runtime.activeDialog and not runtime.loadErrorDialog then
...
package.loaded["wfsuite.app.page_runtime"] = PageRuntime
return PageRuntime
