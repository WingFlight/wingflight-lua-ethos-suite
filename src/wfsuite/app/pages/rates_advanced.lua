...
-- setpoint boost gain/cutoff, all per-axis (Roll/Pitch/Yaw), plus the
-- three yaw-only dynamic fields (ceiling gain, deadband gain, deadband
-- filter). Same MSP_RC_TUNING command as app/pages/rates.lua (see
-- lib/msp_rc_tuning.lua), a different field subset -- matching how the
-- original splits one command's fields across multiple pages by
-- conceptual purpose (same precedent as MSP_PID_PROFILE across
-- app/pages/pid_controller.lua/pid_bandwidth.lua/autolevel.lua).
--
-- No Collective column: axis 4's response_time/accel_limit/
-- setpoint_boost_* fields are wire-present-but-dead on wingflight-
-- firmware (heli-only) -- see lib/msp_rc_tuning.lua's own header.
--
...
-- Column order matches the original's own grid: Roll, Pitch, Yaw -- same
-- axis order every other per-axis grid in this rebuild uses
-- (app/pages/pids.lua/rates.lua).
local COLUMNS = {
...
-- e.g. "response_time_1".."response_time_3").
...
-- column's slot -- same 3-slot line as buildAxisRow() above, so this
-- field lines up under the Yaw header exactly like the axis rows do,
-- just with the other 2 slots left empty.
