-- Servos -> PWM Output.
--
-- Builds the PWM servo list from MSP_STATUS servo_count, then opens an
-- indexed per-servo config editor. Tool toggles servo override; while
-- override is enabled, Center is live-written via SET_SERVO_CENTER and the
...
local function buildRows(servoCount)
  local rows = {}
...
local function openEditor(opts, listState, row)
  ...
  runtime = pageRuntime.new({
    ...
    profileField = "none",
    unloadPackageKeys = {
      "wfsuite.lib.msp_servo_center",
      "wfsuite.lib.msp_servo_config",
      "wfsuite.lib.msp_servo_override",
    },
    onLoaded = function()
  ...
  fieldLayout.buildSingle(runtime, "@i18n(app.modules.servos.trim)@", {key = "trim"})
  ...
  if opts.setWakeupHandler then
    opts.setWakeupHandler(function()
      ...
      if pendingStatus then
        listState.rows = buildRows(pendingStatus.servo_count)
        closeDialog()
