...
-- app/tool.lua.
--
-- Grid layout (row = axis, column = P/I/D/F/B) matches the original
...
local function open(opts)
  local runtime = pageRuntime.new({
    ...
    mspModule = pidTuning,
    opts = opts,
    unloadPackageKeys = {"wfsuite.lib.msp_pid_tuning"},
  })
  ...
  local headerSlots = form.getFieldSlots(headerLine, {0, 0, 0, 0, 0})
  ...
  for _, row in ipairs(ROWS) do
    ...
    local slots = form.getFieldSlots(line, {0, 0, 0, 0, 0})
    for colIndex = 1, #COLUMNS do
      ...
      -- hardcode :step(5) for every P/I/D/F/B cell on every axis.
      -- Cross-checked against rotorflight-lua-ethos-suite's own
      -- tasks/scheduler/msp/api/PID_TUNING.lua (no per-field step
      -- override on any pid_N_X field) and rotorflight-configurator's
      -- own src/tabs/profiles.html (every PID grid cell is step="1") --
