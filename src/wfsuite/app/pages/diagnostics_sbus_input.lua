-- Tools -> Diagnostics -> SBUS-In Status page.
--
-- Live link/channel readout for the SBUS-In Fallback receiver (see
-- app/pages/ports.lua's RX_SBUS_INPUT function and lib/msp_sbus_input_status.lua),
-- for bench-testing that a fallback satellite is wired/bound correctly and
-- to watch it take over when the main RX link is pulled.
--
...
-- cache from diagnostics_elrs_link.lua so a steady/unchanged value doesn't
-- re-touch the widget every tick (AGENTS.md's GC-churn guardrails).
  ...
  common.openReadOnlyPage(opts, PAGE_TITLE, function(ctx)
    ...
    local function applyStatus(data)
      if not data.enabled then
        ...
        if linkField and linkField.color then
          linkField:color(data.linkUp and GREEN or RED)
        end

        local isFallback = data.activeSource == "fallback"
        setFieldValue(activeField, isFallback and T.activeFallback or T.activeMain)
        if activeField and activeField.color then
          activeField:color(isFallback and RED or GREEN)
        end
      end
